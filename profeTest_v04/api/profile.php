...
if($_SERVER['REQUEST_METHOD']==='GET'){
 ...
 if($partner){ $st=db()->prepare('SELECT COALESCE(SUM(amount_usd),0) FROM distributions WHERE partner_id=?'); $st->execute([$partner['id']]); $dist=(float)$st->fetchColumn(); $st=db()->prepare("SELECT COALESCE(SUM(amount_usd),0) FROM withdrawals WHERE partner_id=? AND status='pagado'"); $st->execute([$partner['id']]); $paid=(float)$st->fetchColumn(); $partner['gains_usd']=max(0,round($dist-$paid,2)); }
