...
const titles={dashboard:'Dashboard',perfil:'Mi Perfil',users:'Usuarios',roles:'Roles y permisos',socios:'Socios',referidos:'Referidos',profits:'Ganancias broker',distribuciones:'Distribuciones',capitales:'Niveles de capital',retiros:'Retiros',whatsapp:'WhatsApp',notificaciones:'Notificaciones',auditoria:'Auditoría'};
const entityMap={users:'users',socios:'partners',referidos:'referrals',profits:'broker_profits',distribuciones:'distributions',capitales:'capital_levels',retiros:'withdrawals',whatsapp:'whatsapp_numbers',notificaciones:'notifications',auditoria:'audit_logs'};
...
let state={modules:[],perms:[],notifications:[],options:{},rows:[],filtered:[],page:1,perPage:10,current:'dashboard',user:JSON.parse(document.body.dataset.user||'{}')};
const schemas={
...
async function loadDashboardData(){
 ...
 if(role==='socio'){
  $('#dashStats').innerHTML=`<div class="card"><span>Mi aporte societario</span><strong>US$ ${money(s.capital)}</strong></div><div class="card"><span>Mis ganancias acumuladas</span><strong>US$ ${money(s.gains)}</strong></div><div class="card"><span>Ganancia global por socio</span><strong>US$ ${money(s.per_partner)}</strong></div><div class="card"><span>Mis retiros pendientes</span><strong>${s.pending_withdrawals||0}</strong></div>`;
 } else {
...
async function openForm(entity,row={}){ await loadOptions(); let fields=[...(schemas[entity]||[])]; const isSocio=state.user.role==='socio';
 ...
 let html=`<form id="swForm" class="form-grid">${fields.map(f=>fieldHtml(f,row)).join('')}</form>`;
 if(entity==='withdrawals' && isSocio){
   const p=(state.options.partners||[])[0]||{};
