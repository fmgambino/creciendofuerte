...
============================================================
ACTUALIZACIÓN V07 - LOGIN FONDO VIDEO/IMAGEN EXCLUSIVO
============================================================
Corrección aplicada:
- Fondo login: tipo = Animación CSS: ignora URL e imagen subida y muestra solamente la animación CSS.
- Fondo login: tipo = Imagen: muestra solamente la imagen adjunta; si no hay imagen adjunta, usa la URL configurada.
- Fondo login: tipo = Video: muestra solamente video. Acepta URL directa .mp4/.webm y también enlaces de YouTube.
- Ejemplo válido YouTube: https://www.youtube.com/watch?v=QQ1QOlMRXNs
- Se quitó cualquier card, ticker, moneda o gráfico superpuesto cuando el modo es Imagen o Video para dejar el bloque derecho limpio.

IMPORTANTE SOBRE YOUTUBE:
YouTube no puede reproducirse dentro de la etiqueta HTML <video>. Por eso se agregó detección automática y se convierte internamente a iframe embed autoplay/mute/loop. En algunos navegadores puede verse una demora inicial de carga por políticas de YouTube o bloqueo de terceros.

IMPORTANTE SOBRE IMÁGENES SUBIDAS EN HOSTINGER:
1) Verificar que exista la carpeta /uploads/settings.
2) Permisos recomendados: carpetas 755 o 775; archivos 644.
3) Si Hostinger bloquea la escritura, crear manualmente uploads/settings y volver a subir la imagen desde Configuraciones.
4) Guardar nuevamente Configuraciones luego de cambiar el selector Fondo login: tipo.

PASOS DE ACTUALIZACIÓN EN HOSTINGER:
1) Hacer backup de archivos y base de datos.
2) Subir/reemplazar todos los archivos de este paquete.
3) Conservar si corresponde la carpeta /uploads/profiles y /uploads/settings del servidor.
4) Entrar como SuperAdmin > Configuraciones.
5) Seleccionar Fondo login: tipo = Video para YouTube o URL MP4/WebM; Imagen para imagen adjunta/URL; CSS para animación.
6) Guardar configuración y limpiar caché del navegador/PWA.
