...
$heroVideo = '';
$heroClass = ' bg-'.$bgType;
// Regla estricta: CSS muestra solo animación CSS; imagen muestra solo imagen; video muestra solo video.
if($bgType === 'image') {
  $imageSrc = $bgPath !== '' ? $bgPath : $bgUrl;
  if($imageSrc !== '') { $heroStyle = ' style="background-image:url('.htmlspecialchars($imageSrc, ENT_QUOTES, 'UTF-8').');background-size:cover;background-position:center;background-repeat:no-repeat;"'; }
}
if($bgType === 'video') {
  $videoSrc = $bgUrl !== '' ? $bgUrl : $bgPath;
  if($videoSrc !== '') { $heroVideo = '<video class="login-bg-video" autoplay muted loop playsinline src="'.htmlspecialchars($videoSrc, ENT_QUOTES, 'UTF-8').'"></video>'; }
}
?>
...
<main class="login-shell">
  ...
  <section class="investment-hero<?=$heroClass?>" aria-hidden="true"<?=$heroStyle?>><?=$heroVideo?><div class="ticker"><span>BTC +2.4%</span><span>NASDAQ +0.8%</span><span>ROI mensual</span></div><div class="coin coin-a">US$</div><div class="coin coin-b">%</div><div class="chart-lines"><i></i><i></i><i></i><i></i></div></section>
</main>
