...
'distributions'=>'SELECT d.id,b.profit_date,p.full_name partner,p.capital_usd partner_capital,d.partner_id,d.amount_usd,d.percent_share,d.status,d.created_at FROM distributions d LEFT JOIN broker_profits b ON b.id=d.broker_profit_id JOIN partners p ON p.id=d.partner_id ORDER BY d.id DESC',
...
if(($u['role']??'')==='socio' && $myPartnerId){
 ...
 if($entity==='distributions'){ $sql='SELECT d.id,b.profit_date,p.full_name partner,p.capital_usd partner_capital,d.partner_id,d.amount_usd,d.percent_share,d.status,d.created_at FROM distributions d LEFT JOIN broker_profits b ON b.id=d.broker_profit_id JOIN partners p ON p.id=d.partner_id WHERE d.partner_id=? ORDER BY d.id DESC'; $params=[$myPartnerId]; }
