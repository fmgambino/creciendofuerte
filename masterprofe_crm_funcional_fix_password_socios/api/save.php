...
try{
 ...
 elseif($entity==='partners'){
  $vals=[clean($_POST['partner_code']??('MSTR-'.rand(1000,9999))),clean($_POST['full_name']??''),clean($_POST['email']??''),clean($_POST['phone']??''),(float)($_POST['capital_usd']??0),(float)($_POST['gains_usd']??0),clean($_POST['kyc_status']??'pendiente'),clean($_POST['status']??'active'),clean($_POST['joined_at']??date('Y-m-d'))];
  if($id){ db()->prepare('UPDATE partners SET partner_code=?,full_name=?,email=?,phone=?,capital_usd=?,gains_usd=?,kyc_status=?,status=?,joined_at=? WHERE id=?')->execute([...$vals,$id]); }
  else { db()->prepare('INSERT INTO partners(partner_code,full_name,email,phone,capital_usd,gains_usd,kyc_status,status,joined_at) VALUES(?,?,?,?,?,?,?,?,?)')->execute($vals); notify('Nuevo socio','Se creó el socio '.$vals[1],'socio'); }
 }
