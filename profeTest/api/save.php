...
try{
 ...
 elseif($entity==='partners'){
  ...
  if($id){
    ...
    $pid=(int)db()->lastInsertId(); ensure_partner_user($pid,$vals[1],$vals[2],$vals[9]); notify('Nuevo socio','Se creó el socio '.$vals[1],'socio');
  }
 }
 ...
 elseif($entity==='withdrawals'){
  ...
  if($isSocio){
    ...
    notify('Nuevo retiro solicitado',($u['full_name']??'Un socio').' solicitó retirar US$ '.$amount.'.','retiro');
  } else {
    ...
    if($id){
      ...
      if($old){
        $wasPaid=$old['status']==='pagado'; $nowPaid=$status==='pagado';
        if($wasPaid){ db()->prepare('UPDATE partners SET gains_usd=gains_usd+? WHERE id=?')->execute([(float)$old['amount_usd'],(int)$old['partner_id']]); }
        if($nowPaid){ db()->prepare('UPDATE partners SET gains_usd=GREATEST(gains_usd-?,0) WHERE id=?')->execute([$amount,$pid]); }
      }
    } else {
      db()->prepare('INSERT INTO withdrawals(partner_id,amount_usd,request_type,destination,status) VALUES(?,?,?,?,?)')->execute([$pid,$amount,$type,$dest,$status]); notify('Nuevo retiro solicitado','Se solicitó un retiro.','retiro');
      if($status==='pagado') db()->prepare('UPDATE partners SET gains_usd=GREATEST(gains_usd-?,0) WHERE id=?')->execute([$amount,$pid]);
    }
  }
 }
