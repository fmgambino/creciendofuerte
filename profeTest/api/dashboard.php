...
if(($u['role']??'')==='socio' && $myPartnerId){
  ...
  $st=db()->prepare("SELECT COALESCE(SUM(amount_usd),0) FROM withdrawals WHERE partner_id=? AND status='pagado'"); $st->execute([$myPartnerId]); $paidMine=(float)$st->fetchColumn();
