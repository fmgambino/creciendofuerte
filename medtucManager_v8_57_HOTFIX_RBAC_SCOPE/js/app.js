...
async function init() {
  ...
  const { data:{ session }, error } = await safeGetSession854();
...
async function safeGetSession854(){
  try {
    return await supa.auth.getSession();
  } catch (error) {
    if (/Invalid Refresh Token|Refresh Token Not Found|refresh_token/i.test(error?.message || '')) {
      await supa.auth.signOut({ scope: 'local' }).catch(() => null);
      localStorage.removeItem(`sb-${cfg.SUPABASE_URL?.split('//')[1]?.split('.')[0]}-auth-token`);
...
/* ========================= v8.56 FINAL: assignment scope + ticket->order notifications ========================= */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const E=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const N=(v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').replace(/\b(ing|lic|dr|dra|cp|c p|prof|profesional|tecnico|tecnica)\b/g,' ').replace(/\s+/g,' ').trim();
  ...
  const isSuper=()=>['superadmin','super admin','super user','superuser'].includes(roleN());
  const isAdmin=()=>isSuper()||['admin','administrador'].includes(roleN());
  const isTechRole=(r)=>['tecnico','tecnicos','tecnica','tecnicas','profesional tecnico'].includes(N(r));
  const isTech=()=>isTechRole(role());
  const isRef=()=>['referente','referentes'].includes(roleN());
  const alias=(m)=>{const n=N(m); if(['service orders','ordenes','ordenes de servicio'].includes(n))return'orders'; if(['support tickets','soporte ticket','soporte tickets'].includes(n))return'tickets'; if(['notificaciones'].includes(n))return'notifications'; return n||'dashboard';};
  const fmt=(v)=>{if(!v)return'-'; const d=new Date(v); return Number.isNaN(d.getTime())?String(v):d.toLocaleString('es-AR');};
  const status=(o)=>o?.service_order_statuses?.name||o?.status_name||o?.status||o?.state||'Sin estado';
  const finalOrder=(o)=>/terminad|entregad|cerrad|finaliz|lista p retirar/i.test(status(o));
  const delayed=(o)=>{const d=new Date(o?.received_at||o?.created_at); return !finalOrder(o)&&!Number.isNaN(d.getTime())&&(Date.now()-d.getTime()>7*86400000);};
  const ids=(row)=>[row?.assigned_to,row?.technician_user_id,row?.collaborator_assigned_to,row?.attended_by,row?.created_by,row?.updated_by].filter(Boolean).map(String);
  function meNames(){
    const p=state?.profile||{}, out=[p.full_name,p.email,email().split('@')[0]].filter(Boolean).map(N);
    return Array.from(new Set(out.filter(x=>x&&x.length>2)));
  }
  function nameMatch(a,b){
    const x=N(a), y=N(b); if(!x||!y)return false;
    if(x===y||x.includes(y)||y.includes(x))return true;
    const ax=x.split(' ').filter(t=>t.length>2), ay=y.split(' ').filter(t=>t.length>2);
    if(ax.length>=2&&ax.every(t=>ay.includes(t)))return true;
    if(ay.length>=2&&ay.every(t=>ax.includes(t)))return true;
    return false;
  }
  function assignedOrder(o){
    if(ids(o).includes(uid()))return true;
    const candidates=[o?.professional_technician,o?.technician_name,o?.attended_by_name,o?.technician,o?.assigned_to_name].filter(Boolean);
    return candidates.some(c=>meNames().some(m=>nameMatch(c,m)));
  }
  function assignedTicket(t, orderIds){
    if(ids(t).includes(uid()))return true;
    if(orderIds?.has(String(t?.service_order_id||'')))return true;
    const candidates=[t?.professional_technician,t?.technician_name,t?.assigned_to_name,t?.attended_by_name].filter(Boolean);
    return candidates.some(c=>meNames().some(m=>nameMatch(c,m)));
  }
  function mineAsReferente(row){
    return String(row?.created_by||'')===uid() || String(row?.requester_email||'').toLowerCase()===email() || String(row?.created_by_email||'').toLowerCase()===email();
  }
  function inject856(){
    if(q('#tm-v856-css'))return;
    const st=document.createElement('style'); st.id='tm-v856-css';
    st.textContent=`
      .tm-scope-note{color:var(--muted);font-weight:700;margin:.25rem 0 1rem}
      .stat-card.warn{border-color:#f59e0b!important;box-shadow:inset 0 0 0 1px rgba(245,158,11,.28)}
      .stat-card.danger{border-color:#ef4444!important;box-shadow:inset 0 0 0 1px rgba(239,68,68,.22)}
      .ticket-assignment-preview{padding:12px 14px;border:1px solid rgba(124,92,255,.35);border-radius:12px;background:rgba(124,92,255,.1);margin-top:10px}
      .ticket-assignment-preview small{display:block;color:var(--muted);margin-top:4px}
      .row-actions{display:flex!important;gap:8px!important;justify-content:center!important;align-items:center!important;flex-wrap:nowrap!important}
      .dashboard-main{display:grid!important;grid-template-columns:minmax(320px,.9fr) minmax(560px,1.6fr)!important;gap:16px!important}
      @media(max-width:1100px){.dashboard-main{grid-template-columns:1fr!important}}
    `;
    document.head.appendChild(st);
  }
  window.canModuleV856=function(module,action='view'){
    const m=alias(module), a=String(action||'view');
    if(m==='logout')return true;
    if(isSuper()||isAdmin())return true;
    if(isTech())return ['dashboard','orders','tickets','notifications','profile','inventory'].includes(m)&&['view','export'].includes(a);
    if(isRef())return ['dashboard','orders','tickets','notifications','profile'].includes(m)&&(['view','export'].includes(a)||(m==='tickets'&&a==='create'));
    return ['dashboard','tickets','notifications','profile'].includes(m)&&(['view'].includes(a)||(m==='tickets'&&a==='create'));
  };
  window.canModuleV850=window.canModuleV852=window.canModuleV853=window.canModuleV856;

  window.fetchTechnicians=async function(){
    const {data=[]}=await supa.from('profiles').select('id,full_name,email,role_name,is_active,office,work_days,work_start,work_end').eq('is_active',true).order('full_name');
    return data.filter(p=>isTechRole(p.role_name));
  };
  async function techAvailable(t){
    try{
      const now=new Date(), iso=now.toISOString();
      const {data:ex=[]}=await supa.from('technician_exceptions').select('id').eq('technician_id',t.id).eq('is_active',true).lte('start_at',iso).or(`end_at.is.null,end_at.gte.${iso}`).limit(1);
      if(ex.length)return false;
    }catch(_){}
    return true;
  }
  window.chooseTechnicians=async function(taskName='',qty=1){
    const techs=await window.fetchTechnicians();
    const available=[];
    for(const t of techs){ if(await techAvailable(t))available.push(t); }
    const pool=available.length?available:techs;
    if(!pool.length)return {main:null,coll:null};
    const seed=(N(taskName)+String(Date.now()).slice(0,8)).split('').reduce((a,c)=>a+c.charCodeAt(0),0);
    const main=pool[seed%pool.length];
    let coll=null;
    if(Number(qty)>7&&pool.length>1)coll=pool[(seed+1)%pool.length];
    return {main,coll};
  };

  async function scopedRows856(){
    const [or,tr,ir,ur]=await Promise.all([
      supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(8000),
      supa.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(8000),
    ...
    if(isTech()){
      orders=orders.filter(assignedOrder);
      const orderIds=new Set(orders.map(o=>String(o.id)));
      tickets=tickets.filter(t=>assignedTicket(t,orderIds));
    } else if(isRef()&&!isAdmin()){
      tickets=tickets.filter(mineAsReferente);
      const ticketIds=new Set(tickets.map(t=>String(t.id)));
      orders=orders.filter(o=>mineAsReferente(o)||ticketIds.has(String(o.origin_ticket_id||'')));
    }
    return {orders,tickets,inventory:ir.data||[],users:ur.data||[]};
  }
  window.getScopedRowsV856=scopedRows856;

  function orderActions(o){
    const id=E(o.id);
    const edit=isSuper()?`<button class="icon-mini" title="Editar" onclick="openOrder('${id}')">${ico?.edit||'Editar'}</button>`:'';
    const del=isSuper()?`<button class="icon-mini danger" title="Eliminar" onclick="deleteOrder('${id}')">${ico?.trash||'Eliminar'}</button>`:'';
    const note=isSuper()?`<button class="icon-mini" title="Nota institucional" onclick="makeNote('${id}')">${ico?.note||'Nota'}</button>`:'';
    return `<div class="row-actions"><button class="icon-mini" title="Ver" onclick="viewOrderV853('${id}')">${ico?.view||'Ver'}</button>${edit}<button class="icon-mini" title="Ingreso" onclick="printOrder('${id}','ingreso')">${ico?.print||'Ingreso'}</button><button class="icon-mini" title="Egreso" onclick="printOrder('${id}','entrega')">${ico?.print||'Egreso'}</button>${note}${del}</div>`;
  }
  function ticketActions(t){
    const id=E(t.id);
    const edit=(isSuper()||isRef())?`<button class="icon-mini" title="Editar" onclick="openForm('tickets','${id}')">${ico?.edit||'Editar'}</button>`:'';
    return `<div class="row-actions"><button class="icon-mini" title="Ver" onclick="viewRecord('tickets','${id}')">${ico?.view||'Ver'}</button>${edit}</div>`;
  }
  window.renderOrdersV856=async function(){
    inject856(); page?.('Ordenes de Servicio','Trazabilidad tecnica institucional, sin importes ni conceptos comerciales.');
    const {orders}=await scopedRows856(); state.rows={...(state.rows||{}),orders}; state.selected=state.selected||{}; state.selected.orders=new Set();
    state.pagination=state.pagination||{}; state.pagination.orders=state.pagination.orders||{page:1,size:10};
    const pg=state.pagination.orders; const opts=[5,10,25,50,100,500,1000]; if(!opts.includes(Number(pg.size)))pg.size=10;
    const total=orders.length, pages=Math.max(1,Math.ceil(total/pg.size)); pg.page=Math.min(Math.max(1,pg.page),pages);
    const view=orders.slice((pg.page-1)*pg.size,(pg.page-1)*pg.size+Number(pg.size));
    const scope=isTech()?'Solo ordenes asignadas a mi usuario tecnico.':isRef()?'Solo ordenes originadas por tickets cargados por mi usuario.':'Registro institucional de ordenes de servicio.';
    const pager=`<div class="pager"><label>Listar <select onchange="setOrderPageSize(this.value)">${opts.map(n=>`<option value="${n}" ${Number(pg.size)===n?'selected':''}>${n}</option>`).join('')}</select></label><div><button class="btn" ${pg.page<=1?'disabled':''} onclick="changeOrderPage(-1)">&larr; Atras</button><span>Pagina ${pg.page} de ${pages} · ${total} registros</span><button class="btn" ${pg.page>=pages?'disabled':''} onclick="changeOrderPage(1)">Siguiente &rarr;</button></div></div>`;
    const buttons=`${isSuper()?'<button class="btn primary" onclick="openOrder()">Nueva orden</button><button class="btn" onclick="openStatusManager()">Estados</button><button class="btn" onclick="importCsv(&quot;orders&quot;)">Importar SATMANAGER CSV/MDB</button>':''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button>`;
    q('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Ordenes tecnicas</h2><p>${E(scope)}</p></div><div class="module-actions">${buttons}</div></div><input class="search" placeholder="Buscar por orden, tecnico, oficina, equipo, marca, modelo, serie o falla..." oninput="filterRows(this.value)">${pager}<div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso original</th><th>Oficina/Reparticion</th><th>Profesional tecnico</th><th>Equipo</th><th>Falla / Diagnostico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${view.map(o=>`<tr data-search="${E(Object.values(o).join(' ').toLowerCase())}"><td><b>#${E(o.satmanager_order||o.order_number||'-')}</b></td><td>${E(fmt(o.received_at))}</td><td>${E(o.office||'-')}<br><small>${E(o.address||'')}</small></td><td>${E(o.professional_technician||o.technician_name||'-')}</td><td>${E([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</td><td>${E(o.fault_description||'-')}</td><td><span class="badge">${E(status(o))}</span></td><td>${orderActions(o)}</td></tr>`).join('')||'<tr><td colspan="8">Sin ordenes asignadas.</td></tr>'}</tbody></table></div>${pager}</div>`;
  };
  window.renderOrders=window.renderOrdersV856;
  window.setOrderPageSize=(v)=>{state.pagination=state.pagination||{};state.pagination.orders=state.pagination.orders||{page:1,size:10};state.pagination.orders.size=Number(v)||10;state.pagination.orders.page=1;window.renderOrdersV856();};
  window.changeOrderPage=(d)=>{state.pagination.orders.page+=Number(d||0);window.renderOrdersV856();};

  window.renderTicketsV856=async function(){
    inject856(); page?.('Soporte Ticket',isTech()?'Tickets asignados a mi usuario tecnico.':isRef()?'Tickets cargados por mi usuario referente.':'Tickets recibidos desde soporte institucional.');
    const {tickets}=await scopedRows856(); state.rows={...(state.rows||{}),tickets};
    q('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Soporte Ticket</h2><p>${isTech()?'Solo tickets asignados a mi usuario tecnico.':isRef()?'Solo tickets creados por mi usuario.':'Solicitudes de soporte registradas.'}</p></div><div class="module-actions">${window.canModuleV856('tickets','create')?'<button class="btn primary" onclick="openForm(&quot;tickets&quot;)">Nuevo ticket</button>':''}<button class="btn" onclick="exportCsv('tickets')">Exportar CSV</button><button class="btn" onclick="exportPdf('tickets')">PDF A4</button></div></div><input class="search" placeholder="Buscar tickets..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Ticket</th><th>Solicitante</th><th>Oficina</th><th>Asunto</th><th>Prioridad</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${tickets.map(t=>`<tr data-search="${E(Object.values(t).join(' ').toLowerCase())}"><td><b>${E(t.ticket_number||'-')}</b></td><td>${E(t.requester_name||'-')}</td><td>${E(t.office||'-')}</td><td>${E(t.subject||'-')}</td><td>${E(t.priority||'-')}</td><td><span class="badge">${E(t.status||'-')}</span></td><td>${ticketActions(t)}</td></tr>`).join('')||'<tr><td colspan="7">Sin tickets asignados.</td></tr>'}</tbody></table></div></div>`;
  };

  window.openTicketFormV856=async function(id=null){
    const row=id?(await supa.from('support_tickets').select('*').eq('id',id).maybeSingle()).data:null;
    const html=`<div class="swal-grid">
      <label>Solicitante<input id="f_requester_name" value="${E(row?.requester_name||state?.profile?.full_name||'')}"></label>
      <label>Email<input id="f_requester_email" value="${E(row?.requester_email||email())}"></label>
      <label>Telefono<input id="f_requester_phone" value="${E(row?.requester_phone||state?.profile?.phone||'')}"></label>
      <label>Oficina/Reparticion<input id="f_office" value="${E(row?.office||state?.profile?.office||'')}"></label>
      <label>Area<input id="f_area" value="${E(row?.area||'')}"></label>
      <label>Tipo de servicio<input id="f_service_type" value="${E(row?.service_type||row?.incidence_type||'Soporte tecnico')}"></label>
      <label>Cantidad equipos<input id="f_equipment_count" type="number" min="1" value="${E(row?.equipment_count||1)}"></label>
      <label>Equipo<input id="f_equipment_type" value="${E(row?.equipment_type||'PC')}"></label>
      <label>Marca<input id="f_brand" value="${E(row?.brand||'')}"></label>
      <label>Modelo<input id="f_model" value="${E(row?.model||'')}"></label>
      <label class="full">Asunto<input id="f_subject" value="${E(row?.subject||'')}"></label>
      <label>Prioridad<select id="f_priority"><option ${row?.priority==='Normal'?'selected':''}>Normal</option><option ${row?.priority==='Urgente'?'selected':''}>Urgente</option><option ${row?.priority==='Alta'?'selected':''}>Alta</option></select></label>
      <label>Estado<input id="f_status" value="${E(row?.status||'Pendiente')}"></label>
      <label class="full">Descripcion<textarea id="f_description">${E(row?.description||'')}</textarea></label>
      <div class="full ticket-assignment-preview" id="ticketAssignPreview">Calculando tecnico disponible...</div>
    </div>`;
    const refresh=async()=>{
      const chosen=await window.chooseTechnicians(q('#f_service_type')?.value||'',Number(q('#f_equipment_count')?.value||1));
      const box=q('#ticketAssignPreview');
      if(box)box.innerHTML=chosen.main?`<b>Responsable tecnico:</b> ${E(chosen.main.full_name||chosen.main.email)}${chosen.coll?`<br><b>Colaborador:</b> ${E(chosen.coll.full_name||chosen.coll.email)}`:''}<small>Asignacion aleatoria entre tecnicos activos, excluyendo excepciones vigentes.</small>`:'<b>Sin tecnicos disponibles.</b><small>Se guardara el ticket sin asignacion automatica.</small>';
    };
    const res=await Swal.fire({title:id?'Editar soporte ticket':'Nuevo soporte ticket',html,width:900,showCancelButton:true,confirmButtonText:'Guardar',customClass:{popup:'tm-modal'},didOpen:()=>{refresh();['#f_service_type','#f_equipment_count'].forEach(s=>q(s)?.addEventListener('input',refresh));},preConfirm:()=>({
      requester_name:q('#f_requester_name')?.value||'',requester_email:q('#f_requester_email')?.value||'',requester_phone:q('#f_requester_phone')?.value||'',office:q('#f_office')?.value||'',area:q('#f_area')?.value||'',service_type:q('#f_service_type')?.value||'Soporte tecnico',incidence_type:q('#f_service_type')?.value||'Soporte tecnico',equipment_count:Number(q('#f_equipment_count')?.value||1),equipment_type:q('#f_equipment_type')?.value||'',brand:q('#f_brand')?.value||'',model:q('#f_model')?.value||'',subject:q('#f_subject')?.value||'',priority:q('#f_priority')?.value||'Normal',status:q('#f_status')?.value||'Pendiente',description:q('#f_description')?.value||''
    })});
    if(!res.value)return;
    const chosen=await window.chooseTechnicians(res.value.service_type,res.value.equipment_count);
    const ticketPayload={...res.value,assigned_to:chosen.main?.id||null,technician_user_id:chosen.main?.id||null,collaborator_assigned_to:chosen.coll?.id||null,updated_by:uid()||null};
    if(!id)ticketPayload.created_by=uid()||null;
    let tr=id?await supa.from('support_tickets').update(ticketPayload).eq('id',id).select('*').single():await supa.from('support_tickets').insert(ticketPayload).select('*').single();
    if(tr.error&&/technician_user_id|schema cache|column/i.test(tr.error.message||'')){delete ticketPayload.technician_user_id; tr=id?await supa.from('support_tickets').update(ticketPayload).eq('id',id).select('*').single():await supa.from('support_tickets').insert(ticketPayload).select('*').single();}
    if(tr.error)return Swal.fire({icon:'error',title:'No se pudo guardar ticket',text:tr.error.message});
    if(!id){
      const so={origin_ticket_id:tr.data.id,assigned_to:chosen.main?.id||null,technician_user_id:chosen.main?.id||null,collaborator_assigned_to:chosen.coll?.id||null,professional_technician:chosen.main?.full_name||null,technician_name:chosen.main?.full_name||null,requester_name:res.value.requester_name,requester_email:res.value.requester_email,requester_phone:res.value.requester_phone,office:res.value.office,address:res.value.area||null,service_type:res.value.service_type,equipment_count:res.value.equipment_count,equipment_type:res.value.equipment_type,brand:res.value.brand,model:res.value.model,fault_description:res.value.description||res.value.subject,priority:res.value.priority,source:'SOPORTE_TICKET',created_by:uid()||null,updated_by:uid()||null};
      let os=await supa.from('service_orders').insert(so).select('id,order_number,satmanager_order').single();
      if(os.error&&/technician_user_id|schema cache|column/i.test(os.error.message||'')){delete so.technician_user_id; os=await supa.from('service_orders').insert(so).select('id,order_number,satmanager_order').single();}
      if(!os.error){
        await supa.from('support_tickets').update({service_order_id:os.data.id}).eq('id',tr.data.id);
        const noteBody=`Ticket ${tr.data.ticket_number||''} generado como Orden ${os.data.order_number||os.data.satmanager_order||''}. Oficina: ${res.value.office||'-'}.`;
        const notices=[];
        if(chosen.main?.id)notices.push({title:'Nueva Orden de Servicio asignada',body:noteBody,module:'orders',entity_id:os.data.id,target_user:chosen.main.id,created_by:uid()||null});
        if(chosen.coll?.id)notices.push({title:'Nueva Orden de Servicio asignada como colaborador',body:noteBody,module:'orders',entity_id:os.data.id,target_user:chosen.coll.id,created_by:uid()||null});
        notices.push({title:'Ticket registrado',body:noteBody,module:'tickets',entity_id:tr.data.id,target_user:uid()||null,created_by:uid()||null});
        await supa.from('notifications').insert(notices);
      }
    }
    await Swal.fire({icon:'success',title:'Ticket guardado',timer:1200,showConfirmButton:false});
    route('tickets');
  };
  window.openTicketFormV848=window.openTicketFormV847=window.openTicketFormV844=window.openTicketFormV843=window.openTicketFormV856;
  const prevOpenForm=window.openForm;
  window.openForm=function(key,id=null){ if(alias(key)==='tickets')return window.openTicketFormV856(id); return prevOpenForm?prevOpenForm(key,id):undefined; };

  window.renderDashboardV856=async function(){
    inject856(); page?.('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de ordenes.');
    const rows=await scopedRows856(); state.rows={...(state.rows||{}),...rows};
    const {orders,tickets,inventory,users}=rows, urgent=orders.filter(o=>N(o.priority)==='urgente'), late=orders.filter(delayed), pending=orders.filter(o=>!finalOrder(o)), done=orders.filter(finalOrder);
    const cards=isTech()?[['orders','Ordenes',orders.length,'Asignadas a mi usuario.',''],['urgent','Urgentes',urgent.length,'Prioridad critica','warn'],['delayed','Atrasadas',late.length,'Mas de 7 dias abiertas','danger'],['tickets','Tickets',tickets.length,'Asignados a mi usuario.','']]:isRef()?[['orders','Ordenes',orders.length,'Originadas por mis tickets.',''],['urgent','Urgentes',urgent.length,'Prioridad critica','warn'],['delayed','Atrasadas',late.length,'Mas de 7 dias abiertas','danger'],['tickets','Tickets',tickets.length,'Cargados por mi usuario.','']]:[['users','Usuarios',users.length,'Perfiles autorizados.',''],['orders','Ordenes',orders.length,'Ordenes tecnicas registradas.',''],['urgent','Urgentes',urgent.length,'Prioridad critica','warn'],['delayed','Atrasadas',late.length,'Mas de 7 dias abiertas','danger'],['tickets','Tickets',tickets.length,'Solicitudes recibidas',''],['inventory','Inventario',inventory.length,'Activos e insumos','']];
    const st={}; orders.forEach(o=>{const s=status(o);st[s]=(st[s]||0)+1;});
    const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, d=new Date(cal.year,cal.month,1), first=d.getDay()||7, days=new Date(cal.year,cal.month+1,0).getDate();
    let cells=''; for(let i=1;i<first;i++)cells+='<div class="cal-cell muted"></div>';
    for(let day=1;day<=days;day++){const os=orders.filter(o=>{const x=new Date(o.received_at||o.created_at);return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day;}); cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span class="cal-os" onclick="event.stopPropagation();viewOrderV853('${E(o.id)}')">OS ${E(o.order_number||o.satmanager_order||'')} · ${E(o.office||'-')}</span>`).join('')}</div>`;}
    q('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card ${c[4]}" onclick="showDashCardV853('${c[0]}')"><small>${E(c[1])}</small><strong>${E(c[2])}</strong><span>${E(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech()?'Resumen de ordenes asignadas al profesional tecnico logueado.':isRef()?'Resumen de tickets y ordenes originadas por mi usuario referente.':'Resumen de ordenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${late.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(st).length?Object.entries(st).map(([k,v])=>`<div class="status-row"><span>${E(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin estados registrados.</p>'}</div><div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Ordenes de Servicio</h2><p>${d.toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">&larr;</button><button class="btn" onclick="changeDashboardMonth(1)">&rarr;</button><label>Anio <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver ordenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIE</span><span>JUE</span><span>VIE</span><span>SAB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div></div>`;
  };
  window.renderDashboard=window.renderDashboardV850=window.renderDashboardV852=window.renderDashboardV853=window.renderDashboardV856;
  window.changeDashboardMonth=function(delta){const n=new Date(), c=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1); state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()}; window.renderDashboardV856();};
  window.changeDashboardYear=function(y){const n=new Date(); state.dashboardCalendar=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}; state.dashboardCalendar.year=Number(y)||n.getFullYear(); window.renderDashboardV856();};

  const prevRoute=window.route;
  window.route=async function(p){
    const k=alias(p||'dashboard');
    if(k==='logout')return prevRoute?prevRoute('logout'):undefined;
    if(!q('#content'))window.renderShell?.();
    state.page=k; syncNavActive?.();
    if(!window.canModuleV856(k,'view')){page?.(k,'');q('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este modulo.</p></div>';return;}
    if(k==='dashboard')return window.renderDashboardV856();
    if(k==='orders')return window.renderOrdersV856();
    if(k==='tickets')return window.renderTicketsV856();
    return prevRoute?prevRoute(k):undefined;
  };
  try{route=window.route;renderDashboard=window.renderDashboardV856;renderOrders=window.renderOrdersV856;}catch(_){}
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{inject856(); if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},4200));
  setTimeout(()=>{inject856(); if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},4600);
})();

/* ========================= v8.55 UX/PDF/NOTIFICATIONS CLEAN ========================= */
(function(){
  const $v=(s,r=document)=>r.querySelector(s);
  const Hv=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const strip=(v)=>String(v??'').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();
  const fmtV=(v)=>{if(!v)return'-'; const d=new Date(v); return isNaN(d.getTime())?String(v):d.toLocaleString('es-AR');};
  const roleV=()=>String(state?.profile?.role_name||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  const isSuperV=()=>roleV().includes('superadmin');

  function inject855(){
    if($v('#v855-css'))return;
    const st=document.createElement('style'); st.id='v855-css';
    st.textContent=`
      .note-modal-v855{display:grid;grid-template-columns:190px minmax(0,1fr);gap:18px;align-items:start}
      .note-ai-panel{position:sticky;top:0;border:1px solid rgba(148,163,184,.24);border-radius:16px;padding:12px;background:rgba(15,23,42,.34)}
      .note-ai-panel .btn{width:100%;justify-content:flex-start;margin-bottom:10px;padding:12px 14px;border-radius:12px}
      .note-ai-panel .btn.ai-primary{background:linear-gradient(135deg,#7c5cff,#19d3da)!important;color:white!important;border-color:transparent!important;box-shadow:0 12px 28px rgba(124,92,255,.22)}
      .note-ai-panel .btn svg{width:18px;height:18px;margin-right:8px;flex:0 0 auto}
      .note-main-v855{display:grid;grid-template-columns:1fr 1fr;gap:12px}
      .note-main-v855 .note-textarea{grid-column:1/-1;min-height:300px!important;border-radius:14px!important}
      .note-main-v855 .note-actions{grid-column:1/-1;display:flex;justify-content:flex-start}
      .notifications-v855 .notif-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin:14px 0}
      .notifications-v855 .notif-list-v855{display:grid;gap:10px}
      .notif-card-v855{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid rgba(148,163,184,.22);background:rgba(15,23,42,.46);border-radius:14px;padding:14px 16px;cursor:pointer}
      .notif-card-v855.unread{border-color:rgba(124,92,255,.7);background:linear-gradient(90deg,rgba(124,92,255,.18),rgba(15,23,42,.52))}
      .notif-card-v855 h3{font-size:15px;margin:0 0 4px}.notif-card-v855 p{margin:0 0 6px;color:#cbd5e1;line-height:1.35}
      .notif-card-v855 small{color:#8fb4e8}.notif-card-v855 input{width:16px;height:16px}
      body.light .note-ai-panel,body.light .notif-card-v855{background:#fff;border-color:#d7dee9}
      body.light .notif-card-v855.unread{background:#f4f0ff;border-color:#8b5cf6}
      body.light .notif-card-v855 p{color:#334155}
      @media(max-width:820px){.note-modal-v855{grid-template-columns:1fr}.note-ai-panel{position:static;display:grid;grid-template-columns:1fr 1fr;gap:10px}.note-ai-panel .btn{margin:0}.note-main-v855{grid-template-columns:1fr}}
    `;
    document.head.appendChild(st);
  }

  function defaultNote(o){
    return `A quien corresponda:\n\nPor medio de la presente se solicita la provision de ${o?.accessories||'los insumos necesarios'} para la Orden de Servicio Nro. ${o?.satmanager_order||o?.order_number||'-'}, correspondiente al equipo ${[o?.equipment_type,o?.brand,o?.model,o?.serial_number].filter(Boolean).join(' / ')||'informado'} de ${o?.office||o?.requester_name||'la reparticion solicitante'}.\n\nMotivo tecnico: ${o?.fault_description||'sin descripcion tecnica cargada'}.\n\nInforme / solucion registrada: ${o?.technical_report||o?.solution||o?.observations||'pendiente de informe final'}.\n\nLa solicitud se realiza a efectos de garantizar la continuidad operativa del soporte tecnico institucional.\n\nAtentamente.`;
  }

  window.makeNote=async function(id){
    inject855();
    let o=(state.rows?.orders||[]).find(x=>String(x.id)===String(id));
    if(!o){const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle(); o=r.data;}
    if(!o)return Swal.fire({icon:'info',title:'Orden no encontrada'});
    const offices=(state.rows?.offices||[]).length?state.rows.offices:(await supa.from('offices').select('name').order('name').limit(1000)).data||[];
    window.__noteOrder854=o;
    const signer=`Profesional Tecnico: ${o.professional_technician||o.technician_name||state.profile?.full_name||''}`;
    const html=`<div class="note-modal-v855">
      <aside class="note-ai-panel">
        <button class="btn ai-primary" type="button" onclick="aiDraftNote854('draft')">${ico.ai||''}<span>Redactar<br>con IA</span></button>
        <button class="btn" type="button" onclick="aiDraftNote854('improve')">${ico.ai||''}<span>Mejorar<br>texto</span></button>
      </aside>
      <section class="note-main-v855">
        <div class="note-field"><label>Oficina/Reparticion destino</label><select id="noteOffice">${offices.map(x=>`<option value="${Hv(x.name)}" ${x.name===(o.office||'')?'selected':''}>${Hv(x.name)}</option>`).join('')}<option value="Otra Oficina/Reparticion">Otra Oficina/Reparticion</option></select></div>
        <div class="note-field"><label>Firma institucional</label><select id="noteSigner"><option value="${Hv(signer)}">${Hv(signer)}</option><option value="Director de Direccion de Informatica: Ing. Gerardo Toro">Director de Direccion de Informatica: Ing. Gerardo Toro</option></select></div>
        <textarea id="noteBody" class="note-textarea">${Hv(defaultNote(o))}</textarea>
        <div class="note-actions"><button class="btn" type="button" onclick="printInstitutionalNote()">Imprimir PDF A4</button></div>
      </section>
    </div>`;
    const {value}=await Swal.fire({title:'Nota institucional',html,width:1120,showCancelButton:true,confirmButtonText:'Guardar nota',cancelButtonText:'Cancelar',customClass:{popup:'tm-modal note-wide'},preConfirm:()=>({body:$v('#noteBody')?.value||'',office:$v('#noteOffice')?.value||'',signer:$v('#noteSigner')?.value||''})});
    if(!value)return;
    const full=`Destino: ${value.office}\nFirma: ${value.signer}\n\n${value.body}`;
    const {error}=await supa.from('service_order_notes').insert({service_order_id:id,note_type:'ia',title:'Nota institucional asistida por IA',body:full,created_by:state.user?.id});
    if(error)return Swal.fire({icon:'error',title:'No se pudo guardar',text:error.message});
    Swal.fire({icon:'success',title:'Nota guardada'});
  };

  async function fetchRowsForPdf(key){
    if((state.rows?.[key]||[]).length)return state.rows[key];
    const map={users:'profiles',orders:'service_orders',loans:'loans',tickets:'support_tickets',notifications:'notifications'};
    const table=map[key]||crud?.[key]?.table; if(!table)return [];
    let q=supa.from(table).select(key==='orders'?'*,service_order_statuses(name,color)':'*').limit(1000);
    if(key==='orders')q=q.order('received_at',{ascending:false});
    else if(key==='notifications')q=q.order('created_at',{ascending:false});
    else q=q.order('created_at',{ascending:false});
    const {data}=await q; return data||[];
  }
  const reportDefs={
    users:{title:'Usuarios',orientation:'p',cols:[['full_name','Nombre',36],['email','Email',54],['role_name','Rol',24],['office','Oficina',48],['phone','Telefono',26],['is_active','Estado',22]],map:(r,k)=>k==='is_active'?(r[k]?'Activo':'Inactivo'):r[k]},
    orders:{title:'Ordenes de Servicio',orientation:'l',cols:[['satmanager_order','Orden',16],['received_at','Ingreso',28],['office','Oficina',34],['professional_technician','Tecnico',28],['equipment_type','Equipo',28],['serial_number','Serie',26],['fault_description','Falla / Diagnostico',58],['service_order_statuses','Estado',26]],map:(r,k)=>k==='satmanager_order'?(r.satmanager_order||r.order_number):k==='professional_technician'?(r.professional_technician||r.technician_name):k==='equipment_type'?[r.equipment_type,r.brand,r.model].filter(Boolean).join(' / '):k==='service_order_statuses'?(r.service_order_statuses?.name||r.status||'Sin estado'):k==='received_at'?fmtV(r.received_at):r[k]},
    loans:{title:'Gestion de Prestamos',orientation:'l',cols:[['requester_name','Solicitante',36],['requester_email','Email',44],['office','Oficina',36],['item_description','Insumo/Equipo',54],['quantity','Cant.',14],['status','Estado',24],['start_at','Inicio',28],['due_at','Vencimiento',28],['returned_at','Devolucion',28]],map:(r,k)=>['start_at','due_at','returned_at'].includes(k)?fmtV(r[k]):(k==='item_description'?(r.item_description||r.inventory_items?.name):r[k])},
    tickets:{title:'Soporte Ticket',orientation:'l',cols:[['ticket_number','Ticket',28],['requester_name','Solicitante',36],['office','Oficina',38],['subject','Asunto',48],['priority','Prioridad',22],['status','Estado',24],['description','Descripcion',72]],map:(r,k)=>r[k]},
    notifications:{title:'Notificaciones',orientation:'p',cols:[['created_at','Fecha',34],['title','Titulo',42],['body','Detalle',78],['module','Modulo',22],['is_read','Estado',20]],map:(r,k)=>k==='created_at'?fmtV(r.created_at):k==='is_read'?(r.is_read?'Leida':'Sin leer'):r[k]}
  };
  async function imageData(url){
    try{
      if(!url||String(url).endsWith('.svg'))return null;
  ...
  window.exportPdf=async function(key){
    const {jsPDF}=window.jspdf||{}; if(!jsPDF)return Swal.fire({icon:'error',title:'jsPDF no cargado'});
    const def=reportDefs[key]||reportDefs.notifications, rows=await fetchRowsForPdf(key);
    const doc=new jsPDF(def.orientation,'mm','a4');
    const pageW=doc.internal.pageSize.width, pageH=doc.internal.pageSize.height;
    const logo=await imageData(state.settings?.logo_light_url||state.settings?.logo_dark_url);
    const drawHeader=()=>{
      if(logo)try{doc.addImage(logo,'PNG',14,10,38,16);}catch(_){doc.roundedRect(14,10,38,16,2,2);doc.setFontSize(6);doc.text('Ministerio de\nEducacion Tucuman',18,16);}
      else{doc.roundedRect(14,10,38,16,2,2);doc.setFontSize(6);doc.text('Ministerio de\nEducacion Tucuman',18,16);}
      doc.setFontSize(15);doc.setTextColor(15,23,42);doc.text(`TICKET MANAGER - ${def.title}`,58,17);
      doc.setFontSize(8);doc.text(`Fecha: ${new Date().toLocaleString('es-AR')}  Usuario: ${state.profile?.full_name||'-'}`,58,24);
      doc.setDrawColor(124,92,255);doc.line(14,30,pageW-14,30);
    };
    const head=[def.cols.map(c=>c[1])];
    const body=rows.map(r=>def.cols.map(([k])=>strip(def.map?def.map(r,k):r[k]).slice(0,260)));
    if(doc.autoTable){
      doc.autoTable({
        head, body, startY:36, margin:{left:14,right:14,top:36,bottom:18},
        styles:{fontSize:def.orientation==='l'?7:8,cellPadding:2,overflow:'linebreak',valign:'top'},
        headStyles:{fillColor:[24,42,75],textColor:[255,255,255],fontStyle:'bold'},
        alternateRowStyles:{fillColor:[247,249,252]},
        columnStyles:Object.fromEntries(def.cols.map((c,i)=>[i,{cellWidth:c[2]}])),
        didDrawPage:()=>{drawHeader(); const n=doc.internal.getNumberOfPages(); doc.setFontSize(8);doc.setTextColor(71,85,105);doc.text(`TICKET MANAGER © 2026 Tucuman - Argentina · Pagina ${n}`,14,pageH-8);}
      });
    }else{
      drawHeader(); doc.setFontSize(9); doc.text(body.map(r=>r.join(' | ')).join('\n').slice(0,3500),14,38);
    }
    doc.save(`reporte_${key}.pdf`);
  };

  async function notificationsRows(page,size){
    const from=(page-1)*size, to=from+size-1;
    const {data=[],count=0}=await supa.from('notifications').select('*',{count:'exact'}).order('created_at',{ascending:false}).range(from,to);
    return {data,count};
  }
  window.renderNotificationsV855=async function(){
    inject855();
    page?.('Notificaciones','Avisos automaticos, cambios de estado y auditoria operativa.');
    state.pagination=state.pagination||{}; state.pagination.notifications=state.pagination.notifications||{page:1,size:10};
    const pag=state.pagination.notifications;
    const {data,count}=await notificationsRows(pag.page,pag.size);
    const totalPages=Math.max(1,Math.ceil((count||0)/pag.size)); if(pag.page>totalPages){pag.page=totalPages; return window.renderNotificationsV855();}
    state.rows={...(state.rows||{}),notifications:data}; state.selected.notifications=new Set();
    const pager=`<div class="pager"><label>Listar <select onchange="state.pagination.notifications={page:1,size:Number(this.value)||10};renderNotificationsV855()">${[5,10,25,50,100,500].map(n=>`<option value="${n}" ${Number(pag.size)===n?'selected':''}>${n}</option>`).join('')}</select></label><div><span>Pagina ${pag.page} de ${totalPages} · ${count||0} registros</span><button class="btn" ${pag.page<=1?'disabled':''} onclick="state.pagination.notifications.page--;renderNotificationsV855()">← Atras</button><button class="btn" ${pag.page>=totalPages?'disabled':''} onclick="state.pagination.notifications.page++;renderNotificationsV855()">Siguiente →</button></div></div>`;
    const bulk=`<div class="bulkbar notif-tools"><label class="checkline"><input type="checkbox" onchange="toggleAll('notifications',this.checked)"> Seleccionar todo</label><button class="btn" onclick="markSelectedNotifications(true)">Marcar leidas</button><button class="btn" onclick="markSelectedNotifications(false)">Marcar sin leer</button><button class="btn danger" onclick="bulkDelete('notifications')">Eliminar seleccion</button><button class="btn" onclick="clearNotificationCounter()">Limpiar contador</button><span id="sel_notifications">0 seleccionados</span></div>`;
    $v('#content').innerHTML=`<div class="card notifications-v855"><div class="module-head"><div><h2>Centro de notificaciones</h2><p>Historial operativo con lectura, seleccion masiva y acceso al modulo relacionado.</p></div><div class="module-actions">${isSuperV()?'<button class="btn primary" onclick="sendInternalNotification()">Enviar notificacion</button>':''}<button class="btn" onclick="exportCsv('notifications')">Exportar CSV</button><button class="btn" onclick="exportPdf('notifications')">PDF A4</button></div></div>${pager}${bulk}<div class="notif-list-v855">${data.map(n=>`<article class="notif-card-v855 ${n.is_read?'read':'unread'}" data-id="${Hv(n.id)}" onclick="openNotification('${Hv(n.id)}')"><input type="checkbox" onclick="event.stopPropagation()" onchange="toggleOne('notifications','${Hv(n.id)}',this.checked)"><div><h3>${Hv(n.title)}</h3><p>${Hv(n.body||'')}</p><small>${Hv(fmtV(n.created_at))} · ${Hv(n.module||'-')}</small></div><button class="badge ${n.is_read?'ok':'warn'}" onclick="event.stopPropagation();toggleNotificationRead('${Hv(n.id)}',${!n.is_read})">${n.is_read?'Leida':'Sin leer'}</button></article>`).join('')||'<p>Sin notificaciones.</p>'}</div>${pager}</div>`;
  };
  window.renderNotifications=window.renderNotificationsV855;
  const oldRoute855=window.route;
  window.route=async function(p){
    const k=String(p||'dashboard').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
    if(k==='notifications'||k==='notificaciones')return window.renderNotificationsV855();
    return oldRoute855?oldRoute855(p):undefined;
  };
  try{route=window.route;}catch(_){}
  document.addEventListener('DOMContentLoaded',()=>inject855());
})();

/* ========================= v8.54 CLEAN: realtime, auth hardening, AI notes ========================= */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const H=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const N=(v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const uid=()=>String(state?.user?.id||state?.profile?.id||'');
  const roleN=()=>N(state?.profile?.role_name||'Usuarios');
  const isSuper=()=>['superadmin','super admin'].includes(roleN());
  const aiSvg='<svg viewBox="0 0 24 24" class="icon" aria-hidden="true"><path d="M12 2l1.8 5.2L19 9l-5.2 1.8L12 16l-1.8-5.2L5 9l5.2-1.8L12 2Z"/><path d="M19 14l.9 2.6L22 17.5l-2.1.9L19 21l-.9-2.6-2.1-.9 2.1-.9L19 14Z"/><path d="M5 14l.7 2 2 .7-2 .7-.7 2-.7-2-2-.7 2-.7.7-2Z"/></svg>';
  try{ico.ai=aiSvg; ico.note=ico.note||'<svg viewBox="0 0 24 24" class="icon"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/><path d="M8 13h8M8 17h5"/></svg>';}catch(_){}

  function inject854(){
    if(q('#v854-clean-css'))return;
    const st=document.createElement('style'); st.id='v854-clean-css';
    st.textContent=`
      .ai-note-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:0 0 10px}
      .ai-note-toolbar .btn.ai{background:linear-gradient(135deg,#7c5cff,#19d3da)!important;color:#fff!important;border-color:rgba(255,255,255,.18)!important}
      .ai-note-toolbar .btn svg{width:18px;height:18px;margin-right:6px;vertical-align:-4px}
      .ai-note-hint{display:block;color:#93a4bd;margin:6px 0 0;font-size:12px}
      .note-textarea{min-height:320px!important;line-height:1.55!important}
      .rt-dot{display:inline-flex;width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 4px rgba(34,197,94,.14);margin-left:8px}
    `;
    document.head.appendChild(st);
  }

  function orderById(id){
    return (state.rows?.orders||[]).find(x=>String(x.id)===String(id));
  }
  function orderTitle(o){
    return `Orden de Servicio Nro. ${o?.satmanager_order||o?.order_number||'-'}`;
  }
  function orderEquipment(o){
    return [o?.equipment_type,o?.brand,o?.model,o?.serial_number].filter(Boolean).join(' / ') || 'equipo informado';
  }
  function buildNotePrompt(o, mode, current){
    return {
      mode,
      current_text: current || '',
      order: {
        number: o?.satmanager_order || o?.order_number || '',
        office: o?.office || '',
        address: o?.address || '',
        requester: o?.requester_name || '',
        technician: o?.professional_technician || o?.technician_name || state?.profile?.full_name || '',
        equipment: orderEquipment(o),
        fault: o?.fault_description || '',
        technical_report: o?.technical_report || '',
        solution: o?.solution || o?.observations || '',
        accessories: o?.accessories || '',
        status: o?.service_order_statuses?.name || ''
      }
    };
  }
  function localAIDraft(o, mode, current){
    if(mode==='improve' && current){
      return current
        ...
        .replace(/^\s*/,'')
        .trim()
        + '\n\nSe deja constancia de que la presente se emite con base en la informacion tecnica registrada en la orden, a efectos de garantizar la continuidad operativa del servicio.';
    }
    const needs=(o?.accessories||o?.technical_report||o?.solution||'insumos necesarios para la intervencion tecnica');
    return `A quien corresponda:\n\nPor medio de la presente se solicita la provision de ${needs} para la ${orderTitle(o)}, correspondiente al equipo ${orderEquipment(o)} de ${o?.office||o?.requester_name||'la reparticion solicitante'}.\n\nMotivo tecnico: ${o?.fault_description||'sin descripcion tecnica cargada'}.\n\nInforme / solucion registrada: ${o?.technical_report||o?.solution||o?.observations||'pendiente de informe final'}.\n\nLa solicitud se realiza a efectos de garantizar la continuidad operativa del soporte tecnico institucional y dejar debida trazabilidad administrativa.\n\nAtentamente.`;
  }
  async function aiNoteText(o, mode, current){
    const fallback=localAIDraft(o,mode,current);
    try{
      if(!supa?.functions?.invoke)return fallback;
      const {data,error}=await supa.functions.invoke('ai-note-draft',{body:buildNotePrompt(o,mode,current)});
      if(error||!data?.text)return fallback;
  ...
  window.makeNote=async function(id){
    ...
    let o=orderById(id);
    if(!o){
      const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle();
      o=r.data;
    }
    if(!o)return Swal.fire({icon:'info',title:'Orden no encontrada'});
    const offices=(state.rows?.offices||[]).length?state.rows.offices:(await supa.from('offices').select('name').order('name').limit(1000)).data||[];
    const defaultBody=localAIDraft(o,'draft','');
    const signerDefault=`Profesional Tecnico: ${o.professional_technician||o.technician_name||state.profile?.full_name||''}`;
    const html=`<div class="note-modal-pro">
      <div class="ai-note-toolbar">
        <button class="btn ai" type="button" onclick="aiDraftNote854('draft')">${aiSvg}Redactar con IA</button>
        <button class="btn" type="button" onclick="aiDraftNote854('improve')">${aiSvg}Mejorar texto</button>
        <small class="ai-note-hint">Usa la orden cargada. Si la Edge Function no esta desplegada, aplica asistencia local segura.</small>
      </div>
      <div class="note-field"><label>Oficina/Reparticion destino</label><select id="noteOffice">${offices.map(x=>`<option value="${H(x.name)}" ${x.name===(o.office||'')?'selected':''}>${H(x.name)}</option>`).join('')}<option value="Otra Oficina/Reparticion">Otra Oficina/Reparticion</option></select></div>
      <div class="note-field"><label>Firma institucional</label><select id="noteSigner"><option value="${H(signerDefault)}">${H(signerDefault)}</option><option value="Director de Direccion de Informatica: Ing. Gerardo Toro">Director de Direccion de Informatica: Ing. Gerardo Toro</option></select></div>
      <textarea id="noteBody" class="note-textarea">${H(defaultBody)}</textarea>
      <div class="note-actions"><button class="btn" type="button" onclick="printInstitutionalNote()">Imprimir PDF A4</button></div>
    </div>`;
    window.__noteOrder854=o;
    const {value}=await Swal.fire({title:'Nota institucional',html,width:1120,showCancelButton:true,confirmButtonText:'Guardar nota',cancelButtonText:'Cancelar',customClass:{popup:'tm-modal note-wide'},preConfirm:()=>({body:q('#noteBody')?.value||'',office:q('#noteOffice')?.value||'',signer:q('#noteSigner')?.value||''})});
    if(!value)return;
    const full=`Destino: ${value.office}\nFirma: ${value.signer}\n\n${value.body}`;
    const {error}=await supa.from('service_order_notes').insert({service_order_id:id,note_type:'ia',title:'Nota institucional asistida por IA',body:full,created_by:uid()});
    if(error)return Swal.fire({icon:'error',title:'No se pudo guardar',text:error.message});
    Swal.fire({icon:'success',title:'Nota guardada'});
  };
  window.aiDraftNote854=async function(mode){
    const body=q('#noteBody'); if(!body)return;
    const prev=body.value;
    body.disabled=true;
    try{body.value=await aiNoteText(window.__noteOrder854,mode,prev);}
    finally{body.disabled=false;body.focus();}
  };

  function shouldRefresh(table){
    const p=state?.page||'dashboard';
    return p==='dashboard'||(p==='orders'&&table==='service_orders')||(p==='tickets'&&table==='support_tickets')||(p==='notifications'&&table==='notifications')||(p==='users'&&table==='profiles')||(p==='inventory'&&table==='inventory_items')||(p==='settings'&&table==='app_settings');
  }
  function debounce(fn,ms){let t;return (...a)=>{clearTimeout(t);t=setTimeout(()=>fn(...a),ms);};}
  const refresh=debounce((table)=>{
    if(!state?.profile||!shouldRefresh(table))return;
    if(table==='notifications')window.updateNotificationCountV850?.();
    window.route?.(state.page||'dashboard');
  },450);
  window.subscribeRealtime=function(){
    if(!supa?.channel)return;
    try{if(window.__rt854)supa.removeChannel(window.__rt854);}catch(_){}
    const tables=['profiles','roles','role_module_permissions','permissions','inventory_items','loans','support_tickets','service_orders','service_order_statuses','service_order_history','service_order_notes','notifications','offices','app_settings'];
    const ch=supa.channel('medtuc_realtime_v854');
    tables.forEach(table=>ch.on('postgres_changes',{event:'*',schema:'public',table},payload=>{
      if(table==='notifications'&&payload.new?.title)toast?.(payload.new.title,payload.new.body||'');
      refresh(table);
    }));
    window.__rt854=ch.subscribe();
  };
  try{subscribeRealtime=window.subscribeRealtime;}catch(_){}
  setTimeout(()=>{inject854(); if(state?.profile)window.subscribeRealtime();},4200);
})();

/* ========================= v8.51 FINAL URGENT HOTFIX
   - SuperAdmin always has full module access.
   - Technician scope is strict by assigned user id only.
   - Dashboard cards and calendar order items open robust popups.
   - Notification preview uses client-side filtering to avoid malformed Supabase OR URLs and is scrollable.
   - Technician profile edit cannot modify labor schedule.
============================================================================ */
(function(){
  ...
  const currentRole = () => String(state?.profile?.role_name || 'Usuarios').trim();
  const roleKey = () => normalize(currentRole());
  const currentUserId = () => String(state?.user?.id || state?.profile?.id || '');
  const currentEmail = () => String(state?.profile?.email || state?.user?.email || '').toLowerCase().trim();
  const isSuperAdmin = () => ['superadmin','super admin','superuser','super user'].includes(roleKey());
  const isAdmin = () => isSuperAdmin() || ['admin','administrador'].includes(roleKey());
  const isTech = () => ['tecnico','tecnicos','tecnica','tecnicas','profesional tecnico'].includes(roleKey());
  const isReferente = () => ['referente','referentes'].includes(roleKey());
  const label = {
    dashboard:'Dashboard', users:'Usuarios', roles:'Roles y Permisos', orders:'Ordenes de Servicio',
    inventory:'Inventario', loans:'Gestion de Prestamos', tickets:'Soporte Ticket',
    notifications:'Notificaciones', offices:'Dtos/Oficinas', exceptions:'Excepciones',
    logs:'Registros Historial', profile:'Mi Perfil', settings:'Configuraciones', logout:'Cerrar Sesion'
  };
  const alias = (m) => {
    const n = normalize(m);
    if (['service_orders','ordenes','ordenes de servicio'].includes(n)) return 'orders';
    if (['support_tickets','soporte ticket','soporte_ticket'].includes(n)) return 'tickets';
    if (['role_module_permissions','roles_permissions','permisos'].includes(n)) return 'roles';
    if (['notificaciones'].includes(n)) return 'notifications';
    if (['usuarios'].includes(n)) return 'users';
    if (['prestamos','gestion de prestamos'].includes(n)) return 'loans';
    if (['inventario'].includes(n)) return 'inventory';
    return n || 'dashboard';
  };
  const fmtDate = (v) => {
    if (!v) return '-';
  ...
  const isFinal = (o) => ['terminada','entregada','cerrada','finalizada','lista p/retirar','lista para retirar'].includes(normalize(statusName(o)));
  const isDelayed = (o) => Date.now() - new Date(o?.received_at || o?.created_at || Date.now()).getTime() > 7 * 86400000 && !isFinal(o);
  const assignedOrderToMe = (o) => [o?.assigned_to, o?.technician_user_id, o?.collaborator_assigned_to, o?.attended_by].filter(Boolean).map(String).includes(currentUserId());
  const assignedTicketToMe = (t, assignedOrderIds = new Set()) => [t?.assigned_to, t?.technician_user_id, t?.collaborator_assigned_to, t?.attended_by].filter(Boolean).map(String).includes(currentUserId()) || assignedOrderIds.has(String(t?.service_order_id || ''));

  function injectCss851(){
    if (qs('#v851css')) return;
    const st = document.createElement('style');
    st.id = 'v851css';
    st.textContent = `
      .notification-popover.v851{position:fixed;z-index:999999;width:min(440px,calc(100vw - 24px));max-height:min(680px,calc(100vh - 96px));overflow:hidden;background:linear-gradient(180deg,#13213a,#0d1729);border:1px solid rgba(148,163,184,.3);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.52);padding:12px;color:#f8fafc}
      .notification-popover.v851 .np-head{position:sticky;top:0;z-index:2;display:flex;align-items:center;gap:8px;padding:4px 0 10px;background:#13213a;border-bottom:1px solid rgba(148,163,184,.18)}
      .notification-popover.v851 .np-head b{flex:1}.notification-popover.v851 .np-head button{border:1px solid rgba(148,163,184,.28);background:rgba(255,255,255,.06);color:#fff;border-radius:10px;padding:8px 10px;font-weight:800;cursor:pointer}
      .notification-popover.v851 .np-items{max-height:calc(min(680px,100vh - 96px) - 62px);overflow-y:auto;padding-right:5px;overscroll-behavior:contain}
      .notification-popover.v851 .np-item{display:block;margin:10px 0;padding:12px;border:1px solid rgba(148,163,184,.18);border-radius:12px;background:rgba(15,23,42,.55);cursor:pointer}
      .notification-popover.v851 .np-item b,.notification-popover.v851 .np-item small,.notification-popover.v851 .np-item em{display:block}.notification-popover.v851 .np-item small{margin-top:5px;color:#cbd5e1}.notification-popover.v851 .np-item em{margin-top:6px;color:#93c5fd;font-style:normal;font-size:12px}
      .dash-popup-table{max-height:58vh;overflow:auto;border:1px solid rgba(148,163,184,.25);border-radius:14px}.dash-popup-table table{width:100%;min-width:860px}.dash-actions{display:flex;gap:8px;justify-content:center}
      .calendar-grid .cal-cell span,.calendar-grid .cal-cell button{cursor:pointer}.profile-schedule-locked{opacity:.6;pointer-events:none}.locked-note{grid-column:1/-1;color:#fbbf24;font-weight:800;font-size:12px}
      @media(max-width:760px){.notification-popover.v851{left:10px!important;right:10px!important;top:76px!important;width:auto!important;max-height:72vh}.notification-popover.v851 .np-items{max-height:calc(72vh - 62px)}}
    `;
    document.head.appendChild(st);
  }

  window.canModuleV851 = function(module, action = 'view'){
    const m = alias(module);
    const a = String(action || 'view');
    if (m === 'logout') return true;
    if (isSuperAdmin()) return true;
    if (['dashboard','profile','settings'].includes(m)) return a === 'view' || (m === 'profile' && a === 'edit');
    if (isAdmin()) return true;
    if (isTech()) return ['orders','tickets','notifications'].includes(m) && ['view','export'].includes(a);
    if (isReferente()) return ['dashboard','tickets','notifications','profile'].includes(m) && ['view','create','export'].includes(a);
    return ['dashboard','notifications','profile'].includes(m) && a === 'view';
  };
  window.canModuleV850 = window.canModuleV848 = window.canModuleV847 = window.canModuleV845 = window.canModuleV834 = window.canModuleV830 = window.canModuleV851;

  function moduleList(){
    const src = (typeof modules !== 'undefined' ? modules : window.modules || []).filter(Boolean);
    const visible = src.filter((m) => m[0] !== 'logout' && window.canModuleV851(m[0], 'view'));
    const ensure = (key, icon) => { if (!visible.some((m) => m[0] === key)) visible.push([key, label[key] || key, icon || '']); };
    ensure('dashboard', ico?.dashboard); ensure('profile', ico?.profile); ensure('settings', ico?.settings);
    visible.push(['logout', label.logout, ico?.logout || '']);
    ...
    return visible.filter((m, i, arr) => arr.findIndex((x) => x[0] === m[0]) === i).sort((a,b) => order.indexOf(a[0]) - order.indexOf(b[0]));
  }
  ...
  window.renderShell = function(){
    ...
    const logo = (localStorage.theme === 'light' ? state.settings?.logo_light_url : state.settings?.logo_dark_url) || 'assets/logo.svg';
    const avatar = state.profile?.avatar_url || 'assets/avatar-default.svg';
    const allowed = moduleList();
    const mobileMain = allowed.filter((m) => ['dashboard','orders','tickets','notifications','profile'].includes(m[0])).slice(0,4);
    const themeIcon = localStorage.theme === 'light' ? (typeof themeMoonSvg === 'function' ? themeMoonSvg() : 'M') : (typeof themeSunSvg === 'function' ? themeSunSvg() : 'S');
    qs('#app').innerHTML = `<div class="layout"><aside class="sidebar"><div class="brand" id="brandToggle"><img src="${html(logo)}" onerror="this.src='assets/logo.svg'"><div><strong>${html(state.settings?.institution_name || 'Ministerio de Educacion Tucuman')}</strong><small>${html(state.settings?.institution_area || 'Area Soporte Tecnico')}</small></div></div><nav class="nav">${allowed.map((m) => `<button data-page="${html(m[0])}">${m[2] || ''}<span>${html(label[m[0]] || m[1])}</span></button>`).join('')}</nav><div class="sidebar-foot"><div>TICKET & OS MANAGER © 2026</div><a class="cafecito" href="https://cafecito.app/fmgambino" rel="noopener" target="_blank">Invitame un Cafecito</a></div></aside><main class="main"><section class="topbar"><div class="mobile-brand-top"><img src="${html(logo)}" onerror="this.src='assets/logo.svg'"></div><div class="topbar-title"><h1 id="pageTitle"></h1><p id="pageDesc"></p></div><div class="actions"><button class="icon-btn desktop-only theme-svg" id="themeBtn" title="Cambiar tema">${themeIcon}</button><button class="icon-btn desktop-only" id="fullBtn" title="Pantalla completa">⛶</button><button class="icon-btn notify-trigger" id="notifyBtn" title="Notificaciones">${ico?.bell || ''}<span class="notif-badge-count" hidden>0</span></button><button class="avatar-action" id="avatarBtn" title="Mi perfil"><img src="${html(avatar)}" onerror="this.src='assets/avatar-default.svg'"><span class="desktop-only">${html(state.profile?.full_name || state.profile?.email || '')}</span></button><button class="icon-btn logout-action" id="logoutBtn" title="Cerrar sesion">${ico?.logout || ''}</button></div></section><section id="content"></section><footer class="footer">TICKET MANAGER © 2026 Tucuman - Argentina<br>by <a href="https://github.com/fmgambino" target="_blank" rel="noopener">Ing. Fernando Gambino</a></footer></main><nav class="mobile-dock">${mobileMain.map((m) => `<button data-page="${html(m[0])}">${m[2] || ''}<span>${html(label[m[0]] || m[1])}</span></button>`).join('')}<button id="mobileMoreBtn"><svg class="icon" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h16"/></svg><span>Mas</span></button></nav></div>`;
    qsa('.nav button,.mobile-dock button[data-page]').forEach((b) => b.onclick = () => window.route(b.dataset.page));
    qs('#themeBtn').onclick = () => { localStorage.theme = localStorage.theme === 'light' ? 'dark' : 'light'; applySettings?.(); window.renderShell(); window.route(state.page || 'dashboard'); };
    qs('#fullBtn').onclick = () => document.fullscreenElement ? document.exitFullscreen() : document.documentElement.requestFullscreen?.();
    qs('#avatarBtn').onclick = () => window.route('profile');
    qs('#logoutBtn').onclick = () => window.route('logout');
    qs('#brandToggle').onclick = () => document.body.classList.toggle('sidebar-collapsed');
    qs('#notifyBtn').addEventListener('mouseenter', window.showNotificationPreviewV851);
    qs('#notifyBtn').addEventListener('click', (e) => { e.preventDefault(); window.showNotificationPreviewV851(); });
    qs('#mobileMoreBtn').onclick = () => {
      const rest = allowed.filter((m) => !mobileMain.some((x) => x[0] === m[0]));
      Swal.fire({title:'Menu', html:`<div class="mobile-menu-sheet">${rest.map((m) => `<button class="mobile-menu-item" data-page="${html(m[0])}">${m[2] || ''}<span>${html(label[m[0]] || m[1])}</span></button>`).join('')}</div>`, showConfirmButton:false, showCloseButton:true, customClass:{popup:'tm-modal mobile-menu-modal'}});
      setTimeout(() => qsa('.mobile-menu-item').forEach((b) => b.onclick = () => { Swal.close(); window.route(b.dataset.page); }), 40);
    };
  ...
  async function fetchScopedRows(){
    ...
    let tickets = tr.data || [];
    if (isTech()) {
      orders = orders.filter(assignedOrderToMe);
      const orderIds = new Set(orders.map((o) => String(o.id)));
      tickets = tickets.filter((t) => assignedTicketToMe(t, orderIds));
    } else if (isReferente() && !isAdmin()) {
      tickets = tickets.filter((t) => String(t.created_by || '') === currentUserId() || String(t.requester_email || '').toLowerCase() === currentEmail());
      const ticketIds = new Set(tickets.map((t) => String(t.id)));
      orders = orders.filter((o) => ticketIds.has(String(o.origin_ticket_id || '')) || String(o.requester_email || '').toLowerCase() === currentEmail() || String(o.created_by || '') === currentUserId());
    }
    return {orders, tickets, inventory: ir.data || [], users: ur.data || []};
  }

  window.renderDashboardV851 = async function(){
    injectCss851();
    if (!qs('#content')) window.renderShell();
    page?.('Dashboard', 'Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de ordenes.');
    const rows = await fetchScopedRows();
    state.rows = {...(state.rows || {}), ...rows};
    const {orders, tickets, inventory, users} = rows;
    const urgent = orders.filter((o) => normalize(o.priority) === 'urgente');
    const delayed = orders.filter(isDelayed);
    const pending = orders.filter((o) => !isFinal(o));
    const done = orders.filter(isFinal);
    const cards = isTech() ? [
      ['orders','Ordenes',orders.length,'Asignadas a mi usuario.'], ['urgent','Urgentes',urgent.length,'Prioridad critica'],
      ['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas'], ['tickets','Tickets',tickets.length,'Asignados a mi usuario.']
    ] : [
      ['users','Usuarios',users.length,'Perfiles autorizados.'], ['orders','Ordenes',orders.length,'Ordenes tecnicas registradas.'],
      ['urgent','Urgentes',urgent.length,'Prioridad critica'], ['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas'],
      ['tickets','Tickets',tickets.length,'Solicitudes recibidas'], ['inventory','Inventario',inventory.length,'Activos e insumos']
    ];
    const status = {}; orders.forEach((o) => { const s = statusName(o); status[s] = (status[s] || 0) + 1; });
    qs('#content').innerHTML = `<div class="dash-grid cards">${cards.map((c) => `<div class="stat-card" onclick="showDashCardV851('${c[0]}')"><small>${html(c[1])}</small><strong>${html(c[2])}</strong><span>${html(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech() ? 'Resumen de mis ordenes y tickets asignados.' : 'Resumen de ordenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${delayed.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(status).length ? Object.entries(status).map(([k,v]) => `<div class="status-row"><span>${html(k)}</span><b>${v}</b></div>`).join('') : '<p>Sin ordenes asignadas.</p>'}</div>${calendarHtml(orders)}</div>`;
  };
  window.renderDashboard = window.renderDashboardV850 = window.renderDashboardV851;
  try { renderDashboard = window.renderDashboardV851; } catch(_) {}

  function calendarHtml(orders){
    const now = new Date();
    const cal = state.dashboardCalendar || {month: now.getMonth(), year: now.getFullYear()};
    const d = new Date(cal.year, cal.month, 1);
    const first = d.getDay() || 7;
    const days = new Date(cal.year, cal.month + 1, 0).getDate();
    let cells = '';
    for (let i = 1; i < first; i++) cells += '<div class="cal-cell muted"></div>';
    for (let day = 1; day <= days; day++) {
      const os = (orders || []).filter((o) => { const x = new Date(o.received_at || o.created_at); return x.getFullYear() === cal.year && x.getMonth() === cal.month && x.getDate() === day; });
      cells += `<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map((o) => `<span onclick="event.stopPropagation();viewOrderV851('${html(o.id)}')">OS ${html(o.order_number || o.satmanager_order || '')} · ${html(o.office || '')}</span>`).join('')}</div>`;
    }
    return `<div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Ordenes de Servicio</h2><p>${d.toLocaleString('es-AR', {month:'long', year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map((y) => `<option ${y === cal.year ? 'selected' : ''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver ordenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIE</span><span>JUE</span><span>VIE</span><span>SAB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div>`;
  }
  window.changeDashboardMonth = function(delta){ const n = new Date(); const c = state.dashboardCalendar || {month:n.getMonth(), year:n.getFullYear()}; const d = new Date(c.year, c.month + Number(delta || 0), 1); state.dashboardCalendar = {month:d.getMonth(), year:d.getFullYear()}; window.renderDashboardV851(); };
  window.changeDashboardYear = function(year){ const n = new Date(); state.dashboardCalendar = state.dashboardCalendar || {month:n.getMonth(), year:n.getFullYear()}; state.dashboardCalendar.year = Number(year) || n.getFullYear(); window.renderDashboardV851(); };

  function dashActions(key, id){ return `<div class="dash-actions"><button class="icon-btn" title="Ver" onclick="${key === 'orders' ? `viewOrderV851('${id}')` : `viewRecord('${key}','${id}')`}">${ico?.view || 'Ver'}</button>${window.canModuleV851(key,'edit') ? `<button class="icon-btn" title="Editar" onclick="Swal.close();route('${key}');setTimeout(()=>openForm('${key}','${id}'),300)">${ico?.edit || 'Editar'}</button>` : ''}</div>`; }
  window.showDashCardV851 = function(kind){
    const orders = state.rows?.orders || [], tickets = state.rows?.tickets || [], inv = state.rows?.inventory || [], users = state.rows?.users || [];
    let title = 'Detalle', heads = [], rows = [];
    if (kind === 'users') { title = 'Usuarios contabilizados'; heads = ['Nombre','Email','Rol','Oficina','Acciones']; rows = users.map((u) => [u.full_name,u.email,u.role_name,u.office,dashActions('users',u.id)]); }
    else if (['orders','urgent','delayed','pending','done'].includes(kind)) {
      let arr = orders; if (kind === 'urgent') arr = orders.filter((o) => normalize(o.priority) === 'urgente'); if (kind === 'delayed') arr = orders.filter(isDelayed); if (kind === 'pending') arr = orders.filter((o) => !isFinal(o)); if (kind === 'done') arr = orders.filter(isFinal);
      title = kind === 'urgent' ? 'Ordenes urgentes' : kind === 'delayed' ? 'Ordenes atrasadas' : kind === 'pending' ? 'Ordenes pendientes' : kind === 'done' ? 'Ordenes finalizadas' : 'Ordenes contabilizadas';
      heads = ['Orden','Oficina','Tecnico','Falla / Diagnostico','Acciones']; rows = arr.slice(0,500).map((o) => ['#' + (o.order_number || o.satmanager_order || ''), o.office || '-', o.professional_technician || o.technician_name || '-', o.fault_description || '-', dashActions('orders',o.id)]);
    } else if (kind === 'tickets') { title = 'Tickets contabilizados'; heads = ['Ticket','Solicitante','Oficina','Estado','Acciones']; rows = tickets.slice(0,500).map((t) => [t.ticket_number || '-', t.requester_name || '-', t.office || '-', t.status || '-', dashActions('tickets',t.id)]); }
    else { title = 'Inventario contabilizado'; heads = ['Codigo','Nombre','Categoria','Stock','Acciones']; rows = inv.slice(0,500).map((i) => [i.code || i.barcode || '-', i.name || '-', i.category || i.item_type || '-', i.stock ?? '-', dashActions('inventory',i.id)]); }
    Swal.fire({title, html:`<div class="dash-popup-table"><table><thead><tr>${heads.map((h) => `<th>${html(h)}</th>`).join('')}</tr></thead><tbody>${rows.map((r) => `<tr>${r.map((c,i) => `<td class="${i === r.length - 1 ? 'actions-cell' : ''}">${i === r.length - 1 ? c : html(c)}</td>`).join('')}</tr>`).join('') || `<tr><td colspan="${heads.length}">Sin registros.</td></tr>`}</tbody></table></div><small>Mostrando hasta 500 registros.</small>`, width:1050, confirmButtonText:'OK', customClass:{popup:'tm-modal dash-popup'}});
  };
  window.showDashCard = window.showDashboardPopup = window.showDashCardV850 = window.showDashCardV851;

  window.viewOrderV851 = async function(id){
    let o = (state.rows?.orders || []).find((x) => String(x.id) === String(id));
    if (!o) { const r = await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id', id).maybeSingle(); o = r.data; }
    if (!o) return Swal.fire({icon:'info', title:'Orden no encontrada'});
    if (isTech() && !assignedOrderToMe(o)) return Swal.fire({icon:'warning', title:'Acceso restringido', text:'Esta orden no esta asignada a tu usuario tecnico.'});
    const h = await supa.from('service_order_history').select('*').eq('service_order_id', id).order('created_at', {ascending:false}).limit(20);
    Swal.fire({title:'Detalle de Orden de Servicio', html:`<div class="detail-card order-detail"><div class="order-num">Orden #${html(o.satmanager_order || o.order_number || '-')}</div><div class="detail-grid"><p><b>Ingreso</b><span>${html(fmtDate(o.received_at))}</span></p><p><b>Estado</b><span>${html(statusName(o))}</span></p><p><b>Tecnico</b><span>${html(o.professional_technician || o.technician_name || '-')}</span></p><p><b>Oficina</b><span>${html(o.office || '-')}</span></p><p><b>Solicitante</b><span>${html(o.requester_name || '-')}</span></p><p><b>Equipo</b><span>${html([o.equipment_type,o.brand,o.model].filter(Boolean).join(' · ') || '-')}</span></p><p class="full"><b>Falla / diagnostico</b><span>${html(o.fault_description || '-')}</span></p><p class="full"><b>Informe tecnico</b><span>${html(o.technical_report || '-')}</span></p><p class="full"><b>Solucion / observaciones</b><span>${html(o.solution || o.observations || '-')}</span></p></div><h4>Historial</h4><div class="history-list">${(h.data || []).map((x) => `<div><b>${html(fmtDate(x.created_at))}</b> · ${html(x.action || '')} ${x.new_value ? '→ ' + html(x.new_value) : ''}</div>`).join('') || '<small>Sin movimientos registrados.</small>'}</div></div>`, width:980, customClass:{popup:'tm-modal'}});
  };
  window.viewOrderV850 = window.viewOrder = window.viewOrderV851;

  async function renderOrdersScoped(){
    page?.('Ordenes de Servicio', 'Trazabilidad tecnica institucional.');
    const {orders} = await fetchScopedRows();
    state.rows = {...(state.rows || {}), orders};
    qs('#content').innerHTML = `<div class="card"><div class="module-head"><div><h2>Ordenes tecnicas</h2><p>${isTech() ? 'Solo ordenes asignadas a mi usuario tecnico.' : 'Registro institucional de ordenes de servicio.'}</p></div><div class="module-actions">${isAdmin() ? '<button class="btn primary" onclick="openForm(&quot;orders&quot;)">Nueva orden</button>' : ''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button></div></div><input class="search" placeholder="Buscar en ordenes..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso</th><th>Oficina</th><th>Tecnico</th><th>Equipo</th><th>Falla / Diagnostico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${orders.map((o) => `<tr data-search="${html(Object.values(o).join(' ').toLowerCase())}"><td><b>#${html(o.order_number || o.satmanager_order || '-')}</b></td><td>${html(fmtDate(o.received_at))}</td><td>${html(o.office || '-')}</td><td>${html(o.professional_technician || o.technician_name || '-')}</td><td>${html([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ') || '-')}</td><td>${html(o.fault_description || '-')}</td><td><span class="badge">${html(statusName(o))}</span></td><td class="row-actions">${dashActions('orders', o.id)}</td></tr>`).join('') || '<tr><td colspan="8">Sin ordenes asignadas.</td></tr>'}</tbody></table></div></div>`;
  }

  async function renderTicketsScoped(){
    page?.('Soporte Ticket', isTech() ? 'Tickets asignados a mi usuario tecnico.' : 'Tickets recibidos desde el formulario publico.');
    const rows = await fetchScopedRows();
    const tickets = rows.tickets || [];
    state.rows = {...(state.rows || {}), tickets};
    qs('#content').innerHTML = `<div class="card"><div class="module-head"><div><h2>Soporte Ticket</h2><p>${isTech() ? 'Solo tickets asignados a mi usuario tecnico.' : 'Solicitudes de soporte registradas.'}</p></div><div class="module-actions">${window.canModuleV851('tickets','create') ? '<button class="btn primary" onclick="openForm(&quot;tickets&quot;)">Nuevo</button>' : ''}<button class="btn" onclick="exportCsv('tickets')">Exportar CSV</button><button class="btn" onclick="exportPdf('tickets')">PDF A4</button></div></div><input class="search" placeholder="Buscar tickets..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Ticket</th><th>Solicitante</th><th>Oficina</th><th>Asunto</th><th>Prioridad</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${tickets.map((t) => `<tr data-search="${html(Object.values(t).join(' ').toLowerCase())}"><td><b>${html(t.ticket_number || '-')}</b></td><td>${html(t.requester_name || '-')}</td><td>${html(t.office || '-')}</td><td>${html(t.subject || '-')}</td><td>${html(t.priority || '-')}</td><td><span class="badge">${html(t.status || '-')}</span></td><td class="row-actions">${dashActions('tickets', t.id)}</td></tr>`).join('') || '<tr><td colspan="7">Sin tickets asignados.</td></tr>'}</tbody></table></div></div>`;
  }

  function notificationVisible(n){
    if (isSuperAdmin()) return true;
    const targets = [n.target_role, normalize(n.target_role)].filter(Boolean).map(normalize);
    const singularOrPlural = [roleKey(), roleKey().endsWith('s') ? roleKey().slice(0,-1) : roleKey() + 's'];
    return !n.target_user && !n.target_role || String(n.target_user || '') === currentUserId() || singularOrPlural.some((r) => targets.includes(r));
  }
  async function fetchNotifications(limit = 100, unreadOnly = false){
    const {data = []} = await supa.from('notifications').select('*').order('created_at', {ascending:false}).limit(limit);
    return data.filter((n) => (!unreadOnly || !n.is_read) && notificationVisible(n));
  }
  window.showNotificationPreviewV851 = async function(){
    injectCss851();
    qs('.notification-popover')?.remove();
    const data = await fetchNotifications(100, false);
    const btn = qs('#notifyBtn');
    const r = btn?.getBoundingClientRect?.() || {right: window.innerWidth - 20, bottom: 70};
    const box = document.createElement('div');
    box.className = 'notification-popover v851';
    box.style.right = Math.max(10, window.innerWidth - r.right) + 'px';
    box.style.top = (r.bottom + 8) + 'px';
    box.innerHTML = `<div class="np-head"><b>Notificaciones recientes</b><button onclick="document.querySelector('.notification-popover')?.remove();route('notifications')">Historial</button><button class="np-close" onclick="document.querySelector('.notification-popover')?.remove()">x</button></div><div class="np-items">${data.map((n) => `<div class="np-item" onclick="document.querySelector('.notification-popover')?.remove();route('${html(alias(n.module || 'notifications'))}')"><b>${html(n.title)}</b><small>${html(n.body || '')}</small><em>${html(fmtDate(n.created_at))}</em></div>`).join('') || '<p>Sin notificaciones.</p>'}</div>`;
    box.addEventListener('mouseleave', () => setTimeout(() => box.remove(), 500));
    document.body.appendChild(box);
  };
  window.showNotificationPreview = window.showNotificationPreviewV850 = window.showNotificationPreviewV851;
  window.updateNotificationCountV851 = async function(){
    try {
      const data = await fetchNotifications(500, true);
      const n = data.length;
      qsa('#notifyBtn .notif-badge-count').forEach((b) => { b.textContent = n > 99 ? '99+' : String(n); b.hidden = n === 0; });
    } catch(_) {}
  };
  window.updateNotificationCountV850 = window.updateNotificationCountV851;

  async function renderNotificationsScoped(){
    page?.('Notificaciones', 'Avisos automaticos, cambios de estado y auditoria operativa.');
    const data = await fetchNotifications(5000, false);
    state.rows = {...(state.rows || {}), notifications:data};
    qs('#content').innerHTML = `<div class="card"><div class="module-head"><div><h2>Centro de notificaciones</h2><p>${isSuperAdmin() ? 'Vista global.' : 'Solo notificaciones dirigidas a mi usuario o a mi rol.'}</p></div><div class="module-actions">${isSuperAdmin() ? '<button class="btn primary" onclick="sendInternalNotification()">Enviar notificacion</button>' : ''}<button class="btn" onclick="exportCsv('notifications')">Exportar CSV</button><button class="btn" onclick="exportPdf('notifications')">PDF A4</button></div></div><div class="notification-list">${data.map((n) => `<article class="notification-card ${n.is_read ? 'read' : 'unread'}" onclick="openNotification('${html(n.id)}')"><div><h3>${html(n.title)}</h3><p>${html(n.body || '')}</p><small>${html(fmtDate(n.created_at))} · ${html(n.module || '-')}</small></div><span class="badge ${n.is_read ? 'ok' : 'warn'}">${n.is_read ? 'Leida' : 'Sin leer'}</span></article>`).join('') || '<p>Sin notificaciones.</p>'}</div></div>`;
  }
  window.renderNotifications = window.renderNotificationsV850 = renderNotificationsScoped;

  window.editProfile = async function(){
    const p = state.profile || {};
    const scheduleLocked = !isSuperAdmin();
    const scheduleFields = scheduleLocked ? '' : `<label class="full">Dias laborales<input id="f_work_days" value="${html(Array.isArray(p.work_days) ? p.work_days.join(',') : (p.work_days || ''))}"></label><label>Hora inicio<input id="f_work_start" type="time" value="${html(String(p.work_start || '08:00').slice(0,5))}"></label><label>Hora fin<input id="f_work_end" type="time" value="${html(String(p.work_end || '14:00').slice(0,5))}"></label>`;
    const {value} = await Swal.fire({title:'Editar mi perfil', html:`<div class="swal-grid"><label>Nombre completo<input id="f_full_name" value="${html(p.full_name || '')}"></label><label>Telefono<input id="f_phone" value="${html(p.phone || '')}"></label><label class="full">Oficina<input id="f_office" value="${html(p.office || '')}"></label>${scheduleFields}${scheduleLocked ? '<div class="locked-note">El horario laboral solo puede ser modificado por SuperAdmin.</div>' : ''}</div>`, width:760, showCancelButton:true, confirmButtonText:'Guardar', customClass:{popup:'tm-modal'}, preConfirm:() => ({full_name:qs('#f_full_name')?.value || '', phone:qs('#f_phone')?.value || null, office:qs('#f_office')?.value || null})});
    if (!value) return;
    if (isSuperAdmin()) {
      const days = qs('#f_work_days')?.value;
      if (days !== undefined) value.work_days = days.split(',').map((x) => x.trim()).filter(Boolean);
      value.work_start = qs('#f_work_start')?.value || p.work_start;
      value.work_end = qs('#f_work_end')?.value || p.work_end;
    }
    const {error} = await supa.from('profiles').update({...value, updated_at:new Date().toISOString()}).eq('id', p.id);
    if (error) return Swal.fire({icon:'error', title:'No se pudo guardar', text:error.message});
    await loadProfile?.();
  ...
  window.route = async function(pageKey){
    const k = alias(pageKey || 'dashboard');
    if (k === 'logout') return previousRoute ? previousRoute('logout') : undefined;
    if (!qs('#content')) window.renderShell();
    state.page = k;
    syncNavActive?.();
    if (!window.canModuleV851(k, 'view')) {
      page?.(label[k] || k, '');
      qs('#content').innerHTML = '<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este modulo.</p></div>';
      return;
    }
    if (k === 'dashboard') return window.renderDashboardV851();
    if (k === 'orders') return renderOrdersScoped();
    if (k === 'tickets') return renderTicketsScoped();
    if (k === 'notifications') return renderNotificationsScoped();
...
/* ========================= v8.52 POST-HOTFIX: final override after v8.50 ========================= */
(function(){
  const qs=(s,r=document)=>r.querySelector(s);
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const esc52=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const norm52=(v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const role52=()=>String(state?.profile?.role_name||'Usuarios').trim();
  const roleN52=()=>norm52(role52());
  const uid52=()=>String(state?.user?.id||state?.profile?.id||'');
  const isSuper52=()=>['superadmin','super admin','superuser','super user'].includes(roleN52());
  const isAdmin52=()=>isSuper52()||['admin','administrador'].includes(roleN52());
  const isTech52=()=>['tecnico','tecnicos','tecnica','tecnicas','profesional tecnico'].includes(roleN52());
  const alias52=(m)=>{const n=norm52(m); if(['service_orders','ordenes','ordenes de servicio'].includes(n))return'orders'; if(['support_tickets','soporte ticket','soporte_ticket'].includes(n))return'tickets'; if(['notificaciones'].includes(n))return'notifications'; return n||'dashboard';};
  const fmt52=(v)=>{if(!v)return'-'; const d=new Date(v); return isNaN(d.getTime())?String(v):d.toLocaleString('es-AR');};
  const status52=(o)=>o?.service_order_statuses?.name||o?.status||'Sin estado';
  const final52=(o)=>['terminada','entregada','cerrada','finalizada','lista p/retirar','lista para retirar'].includes(norm52(status52(o)));
  const delayed52=(o)=>Date.now()-new Date(o?.received_at||o?.created_at||Date.now()).getTime()>7*86400000&&!final52(o);
  const assignedOrder52=(o)=>[o?.assigned_to,o?.technician_user_id,o?.collaborator_assigned_to,o?.attended_by].filter(Boolean).map(String).includes(uid52());
  const assignedTicket52=(t,ids=new Set())=>[t?.assigned_to,t?.technician_user_id,t?.collaborator_assigned_to,t?.attended_by].filter(Boolean).map(String).includes(uid52())||ids.has(String(t?.service_order_id||''));

  function css52(){
    if(qs('#v852css'))return;
    const st=document.createElement('style'); st.id='v852css'; st.textContent=`
      .notification-popover{max-height:min(680px,calc(100vh - 96px))!important;overflow:hidden!important}.notification-popover .np-items{max-height:calc(min(680px,100vh - 96px) - 62px)!important;overflow-y:auto!important;overscroll-behavior:contain!important}
      .dash-popup-table{max-height:58vh!important;overflow:auto!important}.calendar-grid .cal-cell span,.calendar-grid .cal-cell button{cursor:pointer!important}
    `; document.head.appendChild(st);
  }

  window.canModuleV852=function(module,action='view'){
    const m=alias52(module), a=String(action||'view');
    if(m==='logout')return true;
    if(isSuper52())return true;
    if(['dashboard','profile','settings'].includes(m))return a==='view'||(m==='profile'&&a==='edit');
    if(isAdmin52())return true;
    if(isTech52())return ['orders','tickets','notifications'].includes(m)&&['view','export'].includes(a);
    return ['dashboard','notifications','profile'].includes(m)&&a==='view';
  };
  window.canModuleV850=window.canModuleV848=window.canModuleV847=window.canModuleV845=window.canModuleV834=window.canModuleV830=window.canModuleV852;

  async function rows52(){
    const [or,tr,ir,ur]=await Promise.all([
      supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000),
      supa.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(5000),
      supa.from('inventory_items').select('*').order('created_at',{ascending:false}).limit(5000),
    ...
    let orders=or.data||[], tickets=tr.data||[];
    if(isTech52()){
      orders=orders.filter(assignedOrder52);
      const ids=new Set(orders.map(o=>String(o.id)));
      tickets=tickets.filter(t=>assignedTicket52(t,ids));
    }
    return {orders,tickets,inventory:ir.data||[],users:ur.data||[]};
  }

  function cardActions52(key,id){return `<div class="dash-actions"><button class="icon-btn" title="Ver" onclick="${key==='orders'?`viewOrderV852('${id}')`:`viewRecord('${key}','${id}')`}">${ico?.view||'Ver'}</button>${window.canModuleV852(key,'edit')?`<button class="icon-btn" title="Editar" onclick="Swal.close();route('${key}');setTimeout(()=>openForm('${key}','${id}'),300)">${ico?.edit||'Editar'}</button>`:''}</div>`;}
  function calendar52(orders){
    const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, d=new Date(cal.year,cal.month,1), first=d.getDay()||7, days=new Date(cal.year,cal.month+1,0).getDate(); let cells='';
    for(let i=1;i<first;i++)cells+='<div class="cal-cell muted"></div>';
    for(let day=1;day<=days;day++){const os=orders.filter(o=>{const x=new Date(o.received_at||o.created_at);return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day}); cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span onclick="event.stopPropagation();viewOrderV852('${esc52(o.id)}')">OS ${esc52(o.order_number||o.satmanager_order||'')} · ${esc52(o.office||'')}</span>`).join('')}</div>`;}
    return `<div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Ordenes de Servicio</h2><p>${d.toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver ordenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIE</span><span>JUE</span><span>VIE</span><span>SAB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div>`;
  }

  window.renderDashboardV852=async function(){
    css52(); if(!qs('#content'))window.renderShell?.(); page?.('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de ordenes.');
    const r=await rows52(); state.rows={...(state.rows||{}),...r}; const {orders,tickets,inventory,users}=r;
    const urgent=orders.filter(o=>norm52(o.priority)==='urgente'), delayed=orders.filter(delayed52), pending=orders.filter(o=>!final52(o)), done=orders.filter(final52);
    const cards=isTech52()?[['orders','Ordenes',orders.length,'Asignadas a mi usuario.'],['urgent','Urgentes',urgent.length,'Prioridad critica'],['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas'],['tickets','Tickets',tickets.length,'Asignados a mi usuario.']]:[['users','Usuarios',users.length,'Perfiles autorizados.'],['orders','Ordenes',orders.length,'Ordenes tecnicas registradas.'],['urgent','Urgentes',urgent.length,'Prioridad critica'],['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas'],['tickets','Tickets',tickets.length,'Solicitudes recibidas'],['inventory','Inventario',inventory.length,'Activos e insumos']];
    const status={}; orders.forEach(o=>{const s=status52(o);status[s]=(status[s]||0)+1});
    qs('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card" onclick="showDashCardV852('${c[0]}')"><small>${esc52(c[1])}</small><strong>${esc52(c[2])}</strong><span>${esc52(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech52()?'Resumen de mis ordenes y tickets asignados.':'Resumen de ordenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${delayed.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(status).length?Object.entries(status).map(([k,v])=>`<div class="status-row"><span>${esc52(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin ordenes asignadas.</p>'}</div>${calendar52(orders)}</div>`;
  };
  window.renderDashboard=window.renderDashboardV850=window.renderDashboardV852;

  window.showDashCardV852=function(kind){
    const orders=state.rows?.orders||[], tickets=state.rows?.tickets||[], inv=state.rows?.inventory||[], users=state.rows?.users||[]; let title='Detalle', heads=[], rows=[];
    if(kind==='users'){title='Usuarios contabilizados';heads=['Nombre','Email','Rol','Oficina','Acciones'];rows=users.map(u=>[u.full_name,u.email,u.role_name,u.office,cardActions52('users',u.id)]);}
    else if(['orders','urgent','delayed','pending','done'].includes(kind)){let arr=orders; if(kind==='urgent')arr=orders.filter(o=>norm52(o.priority)==='urgente'); if(kind==='delayed')arr=orders.filter(delayed52); if(kind==='pending')arr=orders.filter(o=>!final52(o)); if(kind==='done')arr=orders.filter(final52); title=kind==='urgent'?'Ordenes urgentes':kind==='delayed'?'Ordenes atrasadas':kind==='pending'?'Ordenes pendientes':kind==='done'?'Ordenes finalizadas':'Ordenes contabilizadas'; heads=['Orden','Oficina','Tecnico','Falla / Diagnostico','Acciones']; rows=arr.slice(0,500).map(o=>['#'+(o.order_number||o.satmanager_order||''),o.office||'-',o.professional_technician||o.technician_name||'-',o.fault_description||'-',cardActions52('orders',o.id)]);}
    else if(kind==='tickets'){title='Tickets contabilizados';heads=['Ticket','Solicitante','Oficina','Estado','Acciones'];rows=tickets.slice(0,500).map(t=>[t.ticket_number||'-',t.requester_name||'-',t.office||'-',t.status||'-',cardActions52('tickets',t.id)]);}
    else {title='Inventario contabilizado';heads=['Codigo','Nombre','Categoria','Stock','Acciones'];rows=inv.slice(0,500).map(i=>[i.code||i.barcode||'-',i.name||'-',i.category||i.item_type||'-',i.stock??'-',cardActions52('inventory',i.id)]);}
    Swal.fire({title,html:`<div class="dash-popup-table"><table><thead><tr>${heads.map(h=>`<th>${esc52(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map((c,i)=>`<td>${i===r.length-1?c:esc52(c)}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${heads.length}">Sin registros.</td></tr>`}</tbody></table></div><small>Mostrando hasta 500 registros.</small>`,width:1050,confirmButtonText:'OK',customClass:{popup:'tm-modal dash-popup'}});
  };
  window.showDashCard=window.showDashboardPopup=window.showDashCardV850=window.showDashCardV852;

  window.viewOrderV852=async function(id){
    let o=(state.rows?.orders||[]).find(x=>String(x.id)===String(id)); if(!o){const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle(); o=r.data;}
    if(!o)return Swal.fire({icon:'info',title:'Orden no encontrada'}); if(isTech52()&&!assignedOrder52(o))return Swal.fire({icon:'warning',title:'Acceso restringido',text:'Esta orden no esta asignada a tu usuario tecnico.'});
    Swal.fire({title:'Detalle de Orden de Servicio',html:`<div class="detail-card order-detail"><div class="order-num">Orden #${esc52(o.satmanager_order||o.order_number||'-')}</div><div class="detail-grid"><p><b>Ingreso</b><span>${esc52(fmt52(o.received_at))}</span></p><p><b>Estado</b><span>${esc52(status52(o))}</span></p><p><b>Tecnico</b><span>${esc52(o.professional_technician||o.technician_name||'-')}</span></p><p><b>Oficina</b><span>${esc52(o.office||'-')}</span></p><p class="full"><b>Falla / diagnostico</b><span>${esc52(o.fault_description||'-')}</span></p><p class="full"><b>Informe tecnico</b><span>${esc52(o.technical_report||'-')}</span></p><p class="full"><b>Solucion / observaciones</b><span>${esc52(o.solution||o.observations||'-')}</span></p></div></div>`,width:980,customClass:{popup:'tm-modal'}});
  };
  window.viewOrderV850=window.viewOrder=window.viewOrderV852;

  async function renderOrders52(){page?.('Ordenes de Servicio','Trazabilidad tecnica institucional.'); const {orders}=await rows52(); state.rows={...(state.rows||{}),orders}; qs('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Ordenes tecnicas</h2><p>${isTech52()?'Solo ordenes asignadas a mi usuario tecnico.':'Registro institucional de ordenes de servicio.'}</p></div><div class="module-actions">${isAdmin52()?'<button class="btn primary" onclick="openForm(&quot;orders&quot;)">Nueva orden</button>':''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button></div></div><input class="search" placeholder="Buscar en ordenes..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso</th><th>Oficina</th><th>Tecnico</th><th>Equipo</th><th>Falla</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${orders.map(o=>`<tr data-search="${esc52(Object.values(o).join(' ').toLowerCase())}"><td><b>#${esc52(o.order_number||o.satmanager_order||'-')}</b></td><td>${esc52(fmt52(o.received_at))}</td><td>${esc52(o.office||'-')}</td><td>${esc52(o.professional_technician||o.technician_name||'-')}</td><td>${esc52([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</td><td>${esc52(o.fault_description||'-')}</td><td><span class="badge">${esc52(status52(o))}</span></td><td class="row-actions">${cardActions52('orders',o.id)}</td></tr>`).join('')||'<tr><td colspan="8">Sin ordenes asignadas.</td></tr>'}</tbody></table></div></div>`;}
  async function renderTickets52(){page?.('Soporte Ticket',isTech52()?'Tickets asignados a mi usuario tecnico.':'Tickets recibidos desde el formulario publico.'); const {tickets}=await rows52(); state.rows={...(state.rows||{}),tickets}; qs('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Soporte Ticket</h2><p>${isTech52()?'Solo tickets asignados a mi usuario tecnico.':'Solicitudes de soporte registradas.'}</p></div><div class="module-actions">${window.canModuleV852('tickets','create')?'<button class="btn primary" onclick="openForm(&quot;tickets&quot;)">Nuevo</button>':''}<button class="btn" onclick="exportCsv('tickets')">Exportar CSV</button><button class="btn" onclick="exportPdf('tickets')">PDF A4</button></div></div><input class="search" placeholder="Buscar tickets..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Ticket</th><th>Solicitante</th><th>Oficina</th><th>Asunto</th><th>Prioridad</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${tickets.map(t=>`<tr data-search="${esc52(Object.values(t).join(' ').toLowerCase())}"><td><b>${esc52(t.ticket_number||'-')}</b></td><td>${esc52(t.requester_name||'-')}</td><td>${esc52(t.office||'-')}</td><td>${esc52(t.subject||'-')}</td><td>${esc52(t.priority||'-')}</td><td><span class="badge">${esc52(t.status||'-')}</span></td><td class="row-actions">${cardActions52('tickets',t.id)}</td></tr>`).join('')||'<tr><td colspan="7">Sin tickets asignados.</td></tr>'}</tbody></table></div></div>`;}

  async function notif52(limit=100,unread=false){const {data=[]}=await supa.from('notifications').select('*').order('created_at',{ascending:false}).limit(limit); return data.filter(n=>{if(unread&&n.is_read)return false; if(isSuper52())return true; const r=norm52(n.target_role), me=roleN52(), alt=me.endsWith('s')?me.slice(0,-1):me+'s'; return (!n.target_user&&!n.target_role)||String(n.target_user||'')===uid52()||r===me||r===alt;});}
  window.showNotificationPreviewV852=async function(){css52(); qs('.notification-popover')?.remove(); const data=await notif52(100,false); const btn=qs('#notifyBtn'), r=btn?.getBoundingClientRect?.()||{right:window.innerWidth-20,bottom:70}; const box=document.createElement('div'); box.className='notification-popover'; box.style.right=Math.max(10,window.innerWidth-r.right)+'px'; box.style.top=(r.bottom+8)+'px'; box.innerHTML=`<div class="np-head"><b>Notificaciones recientes</b><button onclick="document.querySelector('.notification-popover')?.remove();route('notifications')">Historial</button><button class="np-close" onclick="document.querySelector('.notification-popover')?.remove()">x</button></div><div class="np-items">${data.map(n=>`<div class="np-item" onclick="document.querySelector('.notification-popover')?.remove();route('${esc52(alias52(n.module||'notifications'))}')"><b>${esc52(n.title)}</b><small>${esc52(n.body||'')}</small><em>${esc52(fmt52(n.created_at))}</em></div>`).join('')||'<p>Sin notificaciones.</p>'}</div>`; document.body.appendChild(box);};
  window.showNotificationPreview=window.showNotificationPreviewV850=window.showNotificationPreviewV852;
  window.updateNotificationCountV852=async function(){try{const n=(await notif52(500,true)).length; qsa('#notifyBtn .notif-badge-count').forEach(b=>{b.textContent=n>99?'99+':String(n); b.hidden=n===0;});}catch(_){}};
  window.updateNotificationCountV850=window.updateNotificationCountV852;
  async function renderNotifications52(){page?.('Notificaciones','Avisos automaticos, cambios de estado y auditoria operativa.'); const data=await notif52(5000,false); state.rows={...(state.rows||{}),notifications:data}; qs('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Centro de notificaciones</h2><p>${isSuper52()?'Vista global.':'Solo notificaciones dirigidas a mi usuario o a mi rol.'}</p></div><div class="module-actions">${isSuper52()?'<button class="btn primary" onclick="sendInternalNotification()">Enviar notificacion</button>':''}<button class="btn" onclick="exportCsv('notifications')">Exportar CSV</button><button class="btn" onclick="exportPdf('notifications')">PDF A4</button></div></div><div class="notification-list">${data.map(n=>`<article class="notification-card ${n.is_read?'read':'unread'}" onclick="openNotification('${esc52(n.id)}')"><div><h3>${esc52(n.title)}</h3><p>${esc52(n.body||'')}</p><small>${esc52(fmt52(n.created_at))} · ${esc52(n.module||'-')}</small></div><span class="badge ${n.is_read?'ok':'warn'}">${n.is_read?'Leida':'Sin leer'}</span></article>`).join('')||'<p>Sin notificaciones.</p>'}</div></div>`;}

  window.editProfile=async function(){const p=state.profile||{}; const locked=!isSuper52(); const schedule=locked?'':`<label class="full">Dias laborales<input id="f_work_days" value="${esc52(Array.isArray(p.work_days)?p.work_days.join(','):(p.work_days||''))}"></label><label>Hora inicio<input id="f_work_start" type="time" value="${esc52(String(p.work_start||'08:00').slice(0,5))}"></label><label>Hora fin<input id="f_work_end" type="time" value="${esc52(String(p.work_end||'14:00').slice(0,5))}"></label>`; const {value}=await Swal.fire({title:'Editar mi perfil',html:`<div class="swal-grid"><label>Nombre completo<input id="f_full_name" value="${esc52(p.full_name||'')}"></label><label>Telefono<input id="f_phone" value="${esc52(p.phone||'')}"></label><label class="full">Oficina<input id="f_office" value="${esc52(p.office||'')}"></label>${schedule}${locked?'<div class="locked-note">El horario laboral solo puede ser modificado por SuperAdmin.</div>':''}</div>`,width:760,showCancelButton:true,confirmButtonText:'Guardar',customClass:{popup:'tm-modal'},preConfirm:()=>({full_name:qs('#f_full_name')?.value||'',phone:qs('#f_phone')?.value||null,office:qs('#f_office')?.value||null})}); if(!value)return; if(isSuper52()){const days=qs('#f_work_days')?.value; if(days!==undefined)value.work_days=days.split(',').map(x=>x.trim()).filter(Boolean); value.work_start=qs('#f_work_start')?.value||p.work_start; value.work_end=qs('#f_work_end')?.value||p.work_end;} const {error}=await supa.from('profiles').update({...value,updated_at:new Date().toISOString()}).eq('id',p.id); if(error)return Swal.fire({icon:'error',title:'No se pudo guardar',text:error.message}); await loadProfile?.(); Swal.fire({icon:'success',title:'Perfil actualizado',timer:1200,showConfirmButton:false}); window.renderProfile?.();};

  const prev=window.route;
  window.route=async function(p){const k=alias52(p||'dashboard'); if(k==='logout')return prev?prev('logout'):undefined; if(!qs('#content'))window.renderShell?.(); state.page=k; syncNavActive?.(); if(!window.canModuleV852(k,'view')){page?.(k,''); qs('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este modulo.</p></div>'; return;} if(k==='dashboard')return window.renderDashboardV852(); if(k==='orders')return renderOrders52(); if(k==='tickets')return renderTickets52(); if(k==='notifications')return renderNotifications52(); return prev?prev(k):undefined;};
  try{route=window.route; renderDashboard=window.renderDashboardV852;}catch(_){}
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{css52(); if(state?.profile){window.canModuleV850=window.canModuleV852; window.renderShell?.(); window.route(state.page||'dashboard');}},2200));
  setTimeout(()=>{css52(); if(state?.profile){window.canModuleV850=window.canModuleV852; window.renderShell?.(); window.route(state.page||'dashboard');}},2600);
})();

/* ========================= v8.53 FINAL RESTORE: dashboard/orders UX + safe page ========================= */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const H=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  ...
  const roleN=()=>N(role());
  const uid=()=>String(state?.user?.id||state?.profile?.id||'');
  const isSuper=()=>['superadmin','super admin','superuser','super user'].includes(roleN());
  const isAdmin=()=>isSuper()||['admin','administrador'].includes(roleN());
  const isTech=()=>['tecnico','tecnicos','tecnica','tecnicas','profesional tecnico'].includes(roleN());
  const isRef=()=>['referente','referentes'].includes(roleN());
  const alias=(m)=>{const n=N(m); if(['service_orders','ordenes','ordenes de servicio'].includes(n))return'orders'; if(['support_tickets','soporte ticket','soporte_ticket'].includes(n))return'tickets'; if(['notificaciones'].includes(n))return'notifications'; return n||'dashboard';};
  const dateFmt=(v)=>{if(!v)return'-'; const d=new Date(v); return isNaN(d.getTime())?String(v):d.toLocaleString('es-AR');};
  const statusName=(o)=>o?.service_order_statuses?.name||o?.status||'Sin estado';
  const finalOrder=(o)=>['terminada','entregada','cerrada','finalizada','lista p/retirar','lista para retirar'].includes(N(statusName(o)));
  const delayedOrder=(o)=>Date.now()-new Date(o?.received_at||o?.created_at||Date.now()).getTime()>7*86400000&&!finalOrder(o);
  const techAssignedOrder=(o)=>[o?.assigned_to,o?.technician_user_id,o?.collaborator_assigned_to,o?.attended_by].filter(Boolean).map(String).includes(uid());
  const techAssignedTicket=(t,ids=new Set())=>[t?.assigned_to,t?.technician_user_id,t?.collaborator_assigned_to,t?.attended_by].filter(Boolean).map(String).includes(uid())||ids.has(String(t?.service_order_id||''));
  const orderTech=(o)=> (typeof extractTechFromOrder==='function' ? extractTechFromOrder(o) : '') || o?.professional_technician || o?.technician_name || '-';

  function safePage(title, desc){
    const h=q('#pageTitle'), p=q('#pageDesc');
    if(h) h.textContent=title||'';
    if(p) p.textContent=desc||'';
  }
  window.page=safePage;
  try{ page=safePage; }catch(_){}
  ...
  function inject853(){
    ...
    st.id='v853css';
    st.textContent=`
      *{scrollbar-width:thin;scrollbar-color:rgba(124,92,255,.65) rgba(15,23,42,.35)}
      *::-webkit-scrollbar{width:9px;height:9px}*::-webkit-scrollbar-track{background:rgba(15,23,42,.35);border-radius:999px}*::-webkit-scrollbar-thumb{background:linear-gradient(180deg,rgba(124,92,255,.75),rgba(59,130,246,.45));border-radius:999px;border:2px solid rgba(15,23,42,.35)}
      body.light *{scrollbar-color:rgba(99,102,241,.55) rgba(226,232,240,.7)}body.light *::-webkit-scrollbar-track{background:rgba(226,232,240,.7)}body.light *::-webkit-scrollbar-thumb{border-color:rgba(226,232,240,.7)}
      #content .dash-grid.cards{display:grid!important;grid-template-columns:repeat(6,minmax(140px,1fr))!important;gap:14px!important;margin-bottom:16px!important}
      #content .stat-card{cursor:pointer;transition:transform .15s ease,border-color .15s ease,box-shadow .15s ease}
      #content .stat-card:hover{transform:translateY(-2px);border-color:rgba(124,92,255,.85)!important}
      #content .stat-card.warn{border-color:rgba(245,158,11,.8)!important;box-shadow:inset 0 0 0 1px rgba(245,158,11,.16)}
      #content .stat-card.danger{border-color:rgba(239,68,68,.82)!important;box-shadow:inset 0 0 0 1px rgba(239,68,68,.15)}
      .dashboard-main{display:grid!important;grid-template-columns:minmax(320px,.9fr) minmax(560px,1.6fr)!important;gap:16px!important}
      .dash-popup-table{max-height:58vh!important;overflow:auto!important;border:1px solid rgba(148,163,184,.25)!important;border-radius:14px!important}
      .dash-popup-table table{width:100%!important;min-width:900px!important;table-layout:fixed}.dash-popup-table th,.dash-popup-table td{white-space:normal!important;overflow-wrap:anywhere!important;vertical-align:top!important}
      .dash-actions,.order-detail-actions,.row-actions{display:flex!important;gap:8px!important;align-items:center!important;justify-content:center!important;flex-wrap:nowrap!important}
      .order-detail.with-actions{position:relative;padding-top:52px}.order-detail-actions{position:absolute;right:8px;top:6px}
      .calendar-grid .cal-cell span,.calendar-grid .cal-cell button,.cal-os{cursor:pointer!important}
      .calendar-grid .cal-cell span:hover,.calendar-grid .cal-cell button:hover,.cal-os:hover{filter:brightness(1.14);transform:translateY(-1px)}
      .pager{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}.pager select{min-width:86px}
      @media(max-width:1100px){#content .dash-grid.cards{grid-template-columns:repeat(2,minmax(0,1fr))!important}.dashboard-main{grid-template-columns:1fr!important}}
      @media(max-width:720px){#content .dash-grid.cards{grid-template-columns:1fr!important}.calendar-card{overflow-x:auto}.calendar-grid,.calendar-head{min-width:720px!important}}
    `;
    document.head.appendChild(st);
  }

  window.canModuleV853=function(module,action='view'){
    const m=alias(module), a=String(action||'view');
    if(m==='logout')return true;
    if(isSuper())return true;
    if(['dashboard','profile','settings'].includes(m))return a==='view'||(m==='profile'&&a==='edit');
    if(isAdmin())return true;
    if(isTech())return ['dashboard','orders','tickets','notifications','profile'].includes(m)&&['view','export'].includes(a);
    if(isRef())return ['dashboard','tickets','notifications','profile'].includes(m)&&['view','create','export'].includes(a);
    return ['dashboard','notifications','profile'].includes(m)&&a==='view';
  };
  window.canModuleV850=window.canModuleV852=window.canModuleV853;
  ...
  async function scopedRows(){
    ...
    let orders=or.data||[], tickets=tr.data||[];
    if(isTech()){
      orders=orders.filter(techAssignedOrder);
      const ids=new Set(orders.map(o=>String(o.id)));
      tickets=tickets.filter(t=>techAssignedTicket(t,ids));
    } else if(isRef()&&!isAdmin()){
      const mail=String(state?.profile?.email||'').toLowerCase();
      tickets=tickets.filter(t=>String(t.created_by||'')===uid()||String(t.requester_email||'').toLowerCase()===mail);
      const tids=new Set(tickets.map(t=>String(t.id)));
      orders=orders.filter(o=>tids.has(String(o.origin_ticket_id||''))||String(o.created_by||'')===uid()||String(o.requester_email||'').toLowerCase()===mail);
    }
    return {orders,tickets,inventory:ir.data||[],users:ur.data||[]};
  }

  function orderActionButtons(o, compact=false){
    const id=o.id;
    const edit=isSuper()?`<button class="icon-mini" title="Editar" onclick="Swal.close();openOrder('${id}')">${ico?.edit||'Editar'}</button>`:'';
    const del=isSuper()?`<button class="icon-mini danger" title="Eliminar" onclick="Swal.close();deleteOrder('${id}')">${ico?.trash||'Eliminar'}</button>`:'';
    const note=isSuper()?`<button class="icon-mini" title="Nota institucional" onclick="Swal.close();setTimeout(()=>makeNote('${id}'),120)">${ico?.note||'Nota'}</button>`:'';
    return `<div class="${compact?'order-detail-actions':'row-actions'}"><button class="icon-mini" title="Ver" onclick="viewOrderV853('${id}')">${ico?.view||'Ver'}</button>${edit}<button class="icon-mini" title="Imprimir ingreso" onclick="printOrder('${id}','ingreso')">${ico?.print||'Ingreso'}</button><button class="icon-mini" title="Imprimir egreso" onclick="printOrder('${id}','entrega')">${ico?.print||'Egreso'}</button>${note}${del}</div>`;
  }

  window.viewOrderV853=async function(id){
    let o=(state.rows?.orders||[]).find(x=>String(x.id)===String(id));
    if(!o){const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle(); o=r.data;}
    if(!o)return Swal.fire({icon:'info',title:'Orden no encontrada'});
    if(isTech()&&!techAssignedOrder(o))return Swal.fire({icon:'warning',title:'Acceso restringido',text:'Esta orden no esta asignada a tu usuario tecnico.'});
    const {data:hist=[]}=await supa.from('service_order_history').select('*').eq('service_order_id',id).order('created_at',{ascending:false}).limit(30);
    const actions=`<div class="order-detail-actions"><button class="icon-mini" title="Ir al modulo" onclick="Swal.close();route('orders')">${ico?.orders||'OS'}</button>${isSuper()?`<button class="icon-mini" title="Editar" onclick="Swal.close();openOrder('${id}')">${ico?.edit||'Editar'}</button><button class="icon-mini danger" title="Eliminar" onclick="Swal.close();deleteOrder('${id}')">${ico?.trash||'Eliminar'}</button>`:''}</div>`;
    const body=`<div class="detail-card order-detail with-actions">${actions}<h3>Orden #${H(o.satmanager_order||o.order_number||'-')}</h3><div class="detail-grid"><p><b>Ingreso original</b><span>${H(dateFmt(o.received_at||o.original_received_at))}</span></p><p><b>Estado</b><span>${H(statusName(o))}</span></p><p><b>Profesional tecnico</b><span>${H(orderTech(o))}</span></p><p><b>Oficina/Reparticion</b><span>${H(o.office||'-')}</span></p><p><b>Solicitante / Referente</b><span>${H(o.requester_name||'-')}</span></p><p><b>Direccion</b><span>${H(o.address||'-')}</span></p><p><b>Equipo</b><span>${H([o.equipment_type,o.brand,o.model].filter(Boolean).join(' · ')||'-')}</span></p><p><b>N° serie</b><span>${H(o.serial_number||'-')}</span></p><p><b>Accesorios / Insumos</b><span>${H(o.accessories||'-')}</span></p><p><b>Tabla origen</b><span>${H(o.satmanager_table||'REPARACIONES')}</span></p><p class="full"><b>Falla / Diagnostico</b><span>${H(o.fault_description||'-')}</span></p><p class="full"><b>Informe tecnico</b><span>${H(o.technical_report||'-')}</span></p><p class="full"><b>Solucion / Observaciones</b><span>${H(o.solution||o.observations||'-')}</span></p></div><h4>Historial</h4><div class="history-list">${hist.map(h=>`<div><b>${H(dateFmt(h.created_at))}</b><span>${H(h.action||'')} ${h.previous_value?H(h.previous_value)+' → '+H(h.new_value||''):''}</span></div>`).join('')||'<small>Sin movimientos registrados.</small>'}</div></div>`;
    Swal.fire({title:'Detalle de Orden de Servicio',html:body,width:1040,customClass:{popup:'tm-modal'}});
  };
  window.viewOrder=window.viewOrderV850=window.viewOrderV852=window.viewOrderV853;

  window.renderOrdersV853=async function(){
    inject853(); safePage('Ordenes de Servicio','Trazabilidad tecnica institucional, sin importes ni conceptos comerciales.');
    await loadStatuses?.();
    const {orders}=await scopedRows();
    state.rows={...(state.rows||{}),orders}; state.selected.orders=new Set();
    state.pagination=state.pagination||{}; state.pagination.orders=state.pagination.orders||{page:1,size:10};
    const pg=state.pagination.orders; if(![5,10,25,50,100,500,1000].includes(Number(pg.size)))pg.size=10;
    const total=orders.length, pages=Math.max(1,Math.ceil(total/pg.size)); if(pg.page>pages)pg.page=pages; if(pg.page<1)pg.page=1;
    const start=(pg.page-1)*pg.size, visible=orders.slice(start,start+pg.size);
    const pager=`<div class="pager"><label>Listar <select onchange="setOrderPageSize(this.value)">${[5,10,25,50,100,500,1000].map(n=>`<option value="${n}" ${Number(pg.size)===n?'selected':''}>${n}</option>`).join('')}</select></label><div><button class="btn" ${pg.page<=1?'disabled':''} onclick="changeOrderPage(-1)">← Atras</button><span>Pagina ${pg.page} de ${pages} · ${total} registros</span><button class="btn" ${pg.page>=pages?'disabled':''} onclick="changeOrderPage(1)">Siguiente →</button></div></div>`;
    const bulk=isSuper()?`<div class="bulkbar"><label class="checkline"><input type="checkbox" onchange="toggleAll('orders',this.checked)"> Seleccionar todo</label><button class="btn" onclick="bulkEditOrders()">Cambiar estado</button><button class="btn danger" onclick="bulkDelete('orders')">Eliminar seleccion</button><span id="sel_orders">0 seleccionados</span></div>`:'';
    const buttons=`${isSuper()?'<button class="btn primary" onclick="openOrder()">Nueva orden</button><button class="btn" onclick="openStatusManager()">Estados</button><button class="btn" onclick="importCsv(&quot;orders&quot;)">Importar SATMANAGER CSV/MDB</button>':''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button>`;
    q('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Ordenes tecnicas</h2><p>${isTech()?'Solo ordenes asignadas a mi usuario tecnico.':'Registro de ingreso, profesional tecnico, oficina, estados, informe tecnico, insumos utilizados, notas y entrega.'}</p></div><div class="module-actions">${buttons}</div></div><input class="search" placeholder="Buscar por orden, tecnico, oficina, equipo, marca, modelo, serie o falla..." oninput="filterRows(this.value)">${pager}${bulk}<div class="table-wrap"><table><thead><tr>${isSuper()?'<th class="select-col"></th>':''}<th>Orden</th><th>Ingreso original</th><th>Oficina/Reparticion</th><th>Profesional Tecnico</th><th>Equipo</th><th>Falla / Diagnostico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${visible.map(o=>`<tr data-id="${H(o.id)}" data-search="${H(Object.values(o).join(' ').toLowerCase())}">${isSuper()?`<td class="select-col"><input type="checkbox" onchange="toggleOne('orders','${H(o.id)}',this.checked)"></td>`:''}<td><b>#${H(o.satmanager_order||o.order_number||'-')}</b></td><td>${H(dateFmt(o.received_at))}${o.imported_at?`<br><small>Importado: ${H(dateFmt(o.imported_at))}</small>`:''}</td><td>${H(o.office||'-')}<br><small>${H(o.address||'')}</small></td><td>${H(orderTech(o))}</td><td>${H([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</td><td>${H(o.fault_description||'-')}</td><td><span class="badge" style="color:${H(o.service_order_statuses?.color||'#94a3b8')}">${H(statusName(o))}</span></td><td>${orderActionButtons(o)}</td></tr>`).join('')||`<tr><td colspan="${isSuper()?9:8}">Sin ordenes asignadas.</td></tr>`}</tbody></table></div>${bulk}${pager}</div>`;
  };
  window.renderOrders=window.renderOrdersV853;
  window.setOrderPageSize=(v)=>{state.pagination=state.pagination||{};state.pagination.orders=state.pagination.orders||{page:1,size:10};state.pagination.orders.size=Number(v)||10;state.pagination.orders.page=1;window.renderOrdersV853();};
  window.changeOrderPage=(d)=>{state.pagination.orders.page+=Number(d||0);window.renderOrdersV853();};

  function dashActions(key,id){return `<div class="dash-actions"><button class="icon-mini" title="Ver" onclick="${key==='orders'?`viewOrderV853('${id}')`:`viewRecord('${key}','${id}')`}">${ico?.view||'Ver'}</button>${isSuper()&&key==='orders'?`<button class="icon-mini" title="Editar" onclick="Swal.close();openOrder('${id}')">${ico?.edit||'Editar'}</button><button class="icon-mini danger" title="Eliminar" onclick="Swal.close();deleteOrder('${id}')">${ico?.trash||'Eliminar'}</button>`:''}</div>`;}
  window.showDashCardV853=function(kind){
    const orders=state.rows?.orders||[], tickets=state.rows?.tickets||[], inv=state.rows?.inventory||[], users=state.rows?.users||[];
    let title='Detalle', heads=[], rows=[];
    if(kind==='users'){title='Usuarios contabilizados';heads=['Nombre','Email','Rol','Oficina','Acciones'];rows=users.map(u=>[u.full_name,u.email,u.role_name,u.office,dashActions('users',u.id)]);}
    else if(['orders','urgent','delayed','pending','done'].includes(kind)){let arr=orders;if(kind==='urgent')arr=orders.filter(o=>N(o.priority)==='urgente');if(kind==='delayed')arr=orders.filter(delayedOrder);if(kind==='pending')arr=orders.filter(o=>!finalOrder(o));if(kind==='done')arr=orders.filter(finalOrder);title=kind==='urgent'?'Ordenes urgentes':kind==='delayed'?'Ordenes atrasadas':kind==='pending'?'Ordenes pendientes':kind==='done'?'Ordenes finalizadas':'Ordenes contabilizadas';heads=['Orden','Oficina','Tecnico','Falla / Diagnostico','Acciones'];rows=arr.slice(0,500).map(o=>['#'+(o.satmanager_order||o.order_number||''),o.office||'-',orderTech(o),o.fault_description||'-',dashActions('orders',o.id)]);}
    else if(kind==='tickets'){title='Tickets contabilizados';heads=['Ticket','Solicitante','Oficina','Estado','Acciones'];rows=tickets.slice(0,500).map(t=>[t.ticket_number||'-',t.requester_name||'-',t.office||'-',t.status||'-',dashActions('tickets',t.id)]);}
    else {title='Inventario contabilizado';heads=['Codigo','Nombre','Categoria','Stock','Acciones'];rows=inv.slice(0,500).map(i=>[i.code||i.barcode||'-',i.name||'-',i.category||i.item_type||'-',i.stock??'-',dashActions('inventory',i.id)]);}
    Swal.fire({title,html:`<div class="dashboard-list-popup"><div class="module-actions right"><button class="btn" onclick="Swal.close();route('${kind==='urgent'||kind==='delayed'||kind==='pending'||kind==='done'?'orders':kind}')">Ir al modulo</button></div><div class="dash-popup-table"><table><thead><tr>${heads.map(h=>`<th>${H(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map((c,i)=>`<td>${i===r.length-1?c:H(c)}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${heads.length}">Sin registros.</td></tr>`}</tbody></table></div><small>Mostrando hasta 500 registros.</small></div>`,width:1120,confirmButtonText:'OK',customClass:{popup:'tm-modal dashboard-modal'}});
  };
  window.showDashCard=window.showDashboardPopup=window.showDashCardV850=window.showDashCardV852=window.showDashCardV853;

  window.renderDashboardV853=async function(){
    inject853(); safePage('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de ordenes.');
    const rows=await scopedRows(); state.rows={...(state.rows||{}),...rows};
    const {orders,tickets,inventory,users}=rows;
    const urgent=orders.filter(o=>N(o.priority)==='urgente'), delayed=orders.filter(delayedOrder), pending=orders.filter(o=>!finalOrder(o)), done=orders.filter(finalOrder);
    const cards=isTech()?[
      ['orders','Ordenes',orders.length,'Asignadas a mi usuario.',''],['urgent','Urgentes',urgent.length,'Prioridad critica','warn'],['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas','danger'],['tickets','Tickets',tickets.length,'Asignados a mi usuario.','']
    ]:[
      ['users','Usuarios',users.length,'Perfiles autorizados.',''],['orders','Ordenes',orders.length,'Ordenes tecnicas registradas.',''],['urgent','Urgentes',urgent.length,'Prioridad critica','warn'],['delayed','Atrasadas',delayed.length,'Mas de 7 dias abiertas','danger'],['tickets','Tickets',tickets.length,'Solicitudes recibidas',''],['inventory','Inventario',inventory.length,'Activos e insumos','']
    ];
    const status={}; orders.forEach(o=>{const s=statusName(o);status[s]=(status[s]||0)+1;});
    const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, d=new Date(cal.year,cal.month,1), first=d.getDay()||7, days=new Date(cal.year,cal.month+1,0).getDate();
    let cells=''; for(let i=1;i<first;i++)cells+='<div class="cal-cell muted"></div>';
    for(let day=1;day<=days;day++){const os=orders.filter(o=>{const x=new Date(o.received_at||o.created_at);return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day;});cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span class="cal-os" onclick="event.stopPropagation();viewOrderV853('${H(o.id)}')">OS ${H(o.satmanager_order||o.order_number||'')} · ${H(o.office||'-')}</span>`).join('')}</div>`;}
    q('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card ${c[4]}" onclick="showDashCardV853('${c[0]}')"><small>${H(c[1])}</small><strong>${H(c[2])}</strong><span>${H(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech()?'Resumen de ordenes asignadas al profesional tecnico logueado.':'Resumen de ordenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${delayed.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(status).length?Object.entries(status).map(([k,v])=>`<div class="status-row"><span>${H(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin estados registrados.</p>'}</div><div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Ordenes de Servicio</h2><p>${d.toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver ordenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIE</span><span>JUE</span><span>VIE</span><span>SAB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div></div>`;
  };
  window.renderDashboard=window.renderDashboardV850=window.renderDashboardV852=window.renderDashboardV853;
  window.changeDashboardMonth=function(delta){const n=new Date(), c=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1);state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()};window.renderDashboardV853();};
  window.changeDashboardYear=function(y){const n=new Date();state.dashboardCalendar=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()};state.dashboardCalendar.year=Number(y)||n.getFullYear();window.renderDashboardV853();};
  ...
  window.route=async function(p){
    ...
    if(!q('#content'))window.renderShell?.();
    state.page=k; syncNavActive?.();
    if(!window.canModuleV853(k,'view')){safePage(k,'');q('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este modulo.</p></div>';return;}
    if(k==='dashboard')return window.renderDashboardV853();
    if(k==='orders')return window.renderOrdersV853();
    return prevRoute?prevRoute(k):undefined;
  };
  try{route=window.route;renderDashboard=window.renderDashboardV853;renderOrders=window.renderOrdersV853;}catch(_){}
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{inject853();try{page=safePage;}catch(_){} if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},3200));
  setTimeout(()=>{inject853();try{page=safePage;}catch(_){} if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},3600);
})();

/* ========================= v8.56 FINAL ACTIVATION: keep assignment scope last ========================= */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const N=(v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
  const alias=(m)=>{const n=N(m); if(['service orders','ordenes','ordenes de servicio'].includes(n))return'orders'; if(['support tickets','soporte ticket','soporte tickets'].includes(n))return'tickets'; if(['notificaciones'].includes(n))return'notifications'; return n||'dashboard';};
  const install=()=>{
    if(!window.renderDashboardV856 || !window.renderOrdersV856 || !window.renderTicketsV856) return;
    window.canModuleV850=window.canModuleV852=window.canModuleV853=window.canModuleV856;
    window.renderDashboard=window.renderDashboardV850=window.renderDashboardV852=window.renderDashboardV853=window.renderDashboardV856;
    window.renderOrders=window.renderOrdersV853=window.renderOrdersV856;
    window.openTicketFormV848=window.openTicketFormV847=window.openTicketFormV844=window.openTicketFormV843=window.openTicketFormV856;
    window.route=async function(p){
      const k=alias(p||'dashboard');
      if(k==='logout'){ try{ await supa.auth.signOut(); }catch(_){} location.href='index.html'; return; }
      if(!q('#content')) window.renderShell?.();
      state.page=k; syncNavActive?.();
      if(!window.canModuleV856?.(k,'view')){ page?.(k,''); q('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este modulo.</p></div>'; return; }
      if(k==='dashboard') return window.renderDashboardV856();
      if(k==='orders') return window.renderOrdersV856();
      if(k==='tickets') return window.renderTicketsV856();
      if(k==='notifications' && window.renderNotificationsV855) return window.renderNotificationsV855();
      if(k==='notifications' && window.renderNotificationsV852) return window.renderNotificationsV852();
      return window.__routeBefore856 ? window.__routeBefore856(k) : undefined;
    };
    try{route=window.route;renderDashboard=window.renderDashboardV856;renderOrders=window.renderOrdersV856;}catch(_){}
  };
  if(!window.__routeBefore856) window.__routeBefore856=window.route;
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{install(); if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},5200));
  setTimeout(()=>{install(); if(state?.profile&&q('#app')){window.renderShell?.();window.route(state.page||'dashboard');}},5600);
})();

/* ========================= v8.57 HOTFIX FINAL: anti-flicker + strict technician scope ========================= */
(function(){
  'use strict';
  const $ = (s,r=document)=>r.querySelector(s);
  const E = (v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const N = (v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
  const uid = ()=>String(state?.user?.id||state?.profile?.id||'');
  const role = ()=>N(state?.profile?.role_name||'Usuarios');
  const isSuper = ()=>['superadmin','super admin','superuser','super user'].includes(role());
  const isAdmin = ()=>isSuper()||['admin','administrador'].includes(role());
  const isTech = ()=>['tecnico','tecnicos','tecnica','tecnicas','profesional tecnico'].includes(role());
  const isRef = ()=>['referente','referentes'].includes(role());
  const alias = (m)=>{ const n=N(m); if(['ordenes','ordenes de servicio','service orders','service orders','service orders'].includes(n)||n==='service orders'||n==='service orders'||n==='service orders'||n==='service_orders') return 'orders'; if(['soporte ticket','soporte tickets','support tickets','support_tickets'].includes(n)) return 'tickets'; if(n==='notificaciones') return 'notifications'; return n||'dashboard'; };
  const assignedOrder = (o)=>[o?.assigned_to,o?.technician_user_id,o?.collaborator_assigned_to,o?.attended_by].filter(Boolean).map(String).includes(uid());
  const assignedTicket = (t, orderIds=new Set())=>[t?.assigned_to,t?.technician_user_id,t?.collaborator_assigned_to,t?.attended_by].filter(Boolean).map(String).includes(uid()) || orderIds.has(String(t?.service_order_id||''));
  const statusName = (o)=>o?.service_order_statuses?.name||o?.status||'Sin estado';
  const finalOrder = (o)=>/terminad|entregad|cerrad|finaliz|lista p retirar|lista para retirar/i.test(N(statusName(o)));
  const delayed = (o)=>{ const d=new Date(o?.received_at||o?.created_at); return !finalOrder(o) && !Number.isNaN(d.getTime()) && Date.now()-d.getTime()>7*86400000; };
  const fmt = (v)=>{ if(!v) return '-'; const d=new Date(v); return Number.isNaN(d.getTime())?String(v):d.toLocaleString('es-AR'); };
  const techName = (o)=>o?.professional_technician||o?.technician_name||'-';

  window.canModuleV857 = function(module, action='view'){
    const m=alias(module), a=String(action||'view');
    if(m==='logout') return true;
    if(isAdmin()) return true;
    if(['dashboard','profile','settings'].includes(m)) return a==='view' || (m==='profile' && a==='edit');
    if(isTech()) return ['orders','tickets','notifications','inventory'].includes(m) && ['view','export'].includes(a);
    if(isRef()) return ['orders','tickets','notifications'].includes(m) && (['view','export'].includes(a) || (m==='tickets'&&a==='create'));
    return ['tickets','notifications'].includes(m) && (a==='view'||(m==='tickets'&&a==='create'));
  };
  window.canModuleV856 = window.canModuleV853 = window.canModuleV852 = window.canModuleV850 = window.canModuleV857;

  async function scopedRows857(){
    const [or,tr,ir,ur]=await Promise.all([
      supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000),
      supa.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(5000),
      supa.from('inventory_items').select('*').order('created_at',{ascending:false}).limit(5000),
    ...
    if(isTech()){
      orders=orders.filter(assignedOrder);
      const ids=new Set(orders.map(o=>String(o.id)));
      tickets=tickets.filter(t=>assignedTicket(t,ids));
    } else if(isRef()&&!isAdmin()){
      const mail=String(state?.profile?.email||state?.user?.email||'').toLowerCase();
      tickets=tickets.filter(t=>String(t.created_by||'')===uid() || String(t.requester_email||'').toLowerCase()===mail);
      const tids=new Set(tickets.map(t=>String(t.id)));
      orders=orders.filter(o=>String(o.created_by||'')===uid() || String(o.requester_email||'').toLowerCase()===mail || tids.has(String(o.origin_ticket_id||'')));
    }
    return {orders,tickets,inventory:ir.data||[],users:ur.data||[]};
  }
  window.getScopedRowsV857=scopedRows857;

  window.renderShellV857 = function(){
    const app=$('#app'); if(!app) return;
    const base=[['dashboard','Dashboard',ico.dashboard],['orders','Órdenes de Servicio',ico.orders],['tickets','Soporte Ticket',ico.tickets],['notifications','Notificaciones',ico.bell],['inventory','Inventario',ico.inventory],['users','Usuarios',ico.users],['roles','Roles y Permisos',ico.roles],['offices','Dtos/Oficinas',ico.offices],['loans','Gestión de Préstamos',ico.loans],['profile','Mi Perfil',ico.profile],['settings','Configuraciones',ico.settings],['logout','Cerrar Sesión',ico.logout]];
    const nav=base.filter(([k])=>window.canModuleV857(k,'view') || k==='logout');
    app.innerHTML=`<aside class="sidebar"><div class="brand"><img src="${E((state.settings||{}).logo_dark_url||'assets/logo.svg')}" alt="Logo"><div><strong>${E((state.settings||{}).institution_name||'Ministerio de Educación Tucumán')}</strong><span>${E((state.settings||{}).institution_area||'Dirección de Informática - Área Soporte Técnico')}</span></div></div><nav class="nav">${nav.map(([k,t,i])=>`<button data-page="${k}" class="${state.page===k?'active':''}" onclick="route('${k}')">${i||''}<span>${E(t)}</span></button>`).join('')}</nav><div class="sidebar-foot">TICKET & OS MANAGER © 2026<br><a href="#">Invitame un Cafecito</a></div></aside><main class="main"><header class="topbar"><div><h1 id="pageTitle"></h1><p id="pageDesc"></p></div><div class="top-actions"><button class="icon-btn" onclick="toggleTheme()" title="Modo claro/oscuro">${ico.sun||'☼'}</button><button class="icon-btn" onclick="route(state.page||'dashboard')" title="Actualizar">↻</button><button id="notifyBtn" class="icon-btn" onclick="route('notifications')">${ico.bell||'Notificaciones'}<span id="notifyCount" class="notify-count"></span></button><button id="avatarBtn" class="profile-chip" onclick="changeAvatar()"><img src="${E(state.profile?.avatar_url||'assets/avatar-default.svg')}" onerror="this.src='assets/avatar-default.svg'"><b>${E(state.profile?.full_name||state.user?.email||'Usuario')}</b></button><button class="icon-btn" onclick="route('logout')" title="Cerrar sesión">${ico.logout||'Salir'}</button></div></header><section id="content" class="content"></section><footer class="main-footer">TICKET MANAGER © 2026 Tucumán - Argentina<br>by <u>Ing. Fernando Gambino</u> · Todos los Derechos Registrados</footer></main>`;
    loadNotificationCount?.();
  };
  window.renderShell = window.renderShellV857;
  try{ renderShell=window.renderShellV857; }catch(_){ }

  function cardActions(key,id){ return `<div class="row-actions"><button class="icon-mini" title="Ver" onclick="${key==='orders'?`viewOrderV857('${id}')`:`viewRecord('${key}','${id}')`}">${ico.view||'Ver'}</button>${isAdmin()&&key==='orders'?`<button class="icon-mini" title="Editar" onclick="openOrder('${id}')">${ico.edit||'Editar'}</button><button class="icon-mini danger" title="Eliminar" onclick="deleteOrder('${id}')">${ico.trash||'Eliminar'}</button>`:''}</div>`; }
  window.viewOrderV857 = async function(id){
    let o=(state.rows?.orders||[]).find(x=>String(x.id)===String(id));
    if(!o){ const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle(); o=r.data; }
    if(!o) return Swal.fire({icon:'info',title:'Orden no encontrada'});
    if(isTech()&&!assignedOrder(o)) return Swal.fire({icon:'warning',title:'Acceso restringido',text:'Esta orden no está asignada a tu usuario técnico.'});
    Swal.fire({title:'Detalle de Orden de Servicio',html:`<div class="detail-card order-detail"><h3>Orden #${E(o.satmanager_order||o.order_number||'-')}</h3><div class="detail-grid"><p><b>Ingreso</b><span>${E(fmt(o.received_at||o.original_received_at))}</span></p><p><b>Estado</b><span>${E(statusName(o))}</span></p><p><b>Profesional técnico</b><span>${E(techName(o))}</span></p><p><b>Oficina/Repartición</b><span>${E(o.office||'-')}</span></p><p><b>Equipo</b><span>${E([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</span></p><p class="full"><b>Falla / Diagnóstico</b><span>${E(o.fault_description||'-')}</span></p><p class="full"><b>Informe técnico</b><span>${E(o.technical_report||'-')}</span></p><p class="full"><b>Solución / Observaciones</b><span>${E(o.solution||o.observations||'-')}</span></p></div></div>`,width:980,customClass:{popup:'tm-modal'}});
  };
  window.viewOrder=window.viewOrderV850=window.viewOrderV852=window.viewOrderV853=window.viewOrderV857;

  window.renderOrdersV857 = async function(){
    page?.('Órdenes de Servicio','Trazabilidad técnica institucional.');
    const {orders}=await scopedRows857(); state.rows={...(state.rows||{}),orders}; state.selected.orders=new Set();
    state.pagination=state.pagination||{}; state.pagination.orders=state.pagination.orders||{page:1,size:10};
    const pg=state.pagination.orders; const total=orders.length; const pages=Math.max(1,Math.ceil(total/pg.size)); pg.page=Math.min(Math.max(1,pg.page),pages); const visible=orders.slice((pg.page-1)*pg.size,(pg.page-1)*pg.size+pg.size);
    const pager=`<div class="pager"><label>Listar <select onchange="setOrderPageSize(this.value)">${[5,10,25,50,100,500].map(n=>`<option value="${n}" ${Number(pg.size)===n?'selected':''}>${n}</option>`).join('')}</select></label><div><button class="btn" ${pg.page<=1?'disabled':''} onclick="changeOrderPage(-1)">← Atrás</button><span>Página ${pg.page} de ${pages} · ${total} registros</span><button class="btn" ${pg.page>=pages?'disabled':''} onclick="changeOrderPage(1)">Siguiente →</button></div></div>`;
    const buttons=`${isAdmin()?'<button class="btn primary" onclick="openOrder()">Nueva orden</button><button class="btn" onclick="openStatusManager()">Estados</button><button class="btn" onclick="importCsv(&quot;orders&quot;)">Importar CSV/MDB</button>':''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button>`;
    $('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Órdenes técnicas</h2><p>${isTech()?'Solo órdenes asignadas a mi usuario técnico.':'Registro institucional de órdenes de servicio.'}</p></div><div class="module-actions">${buttons}</div></div><input class="search" placeholder="Buscar por orden, técnico, oficina, equipo, marca, modelo, serie o falla..." oninput="filterRows(this.value)">${pager}<div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso</th><th>Oficina/Repartición</th><th>Profesional técnico</th><th>Equipo</th><th>Falla / Diagnóstico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${visible.map(o=>`<tr data-search="${E(Object.values(o).join(' ').toLowerCase())}"><td><b>#${E(o.satmanager_order||o.order_number||'-')}</b></td><td>${E(fmt(o.received_at))}</td><td>${E(o.office||'-')}<br><small>${E(o.address||'')}</small></td><td>${E(techName(o))}</td><td>${E([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</td><td>${E(o.fault_description||'-')}</td><td><span class="badge" style="color:${E(o.service_order_statuses?.color||'#94a3b8')}">${E(statusName(o))}</span></td><td>${cardActions('orders',o.id)}</td></tr>`).join('')||'<tr><td colspan="8">Sin órdenes asignadas.</td></tr>'}</tbody></table></div>${pager}</div>`;
  };
  window.renderOrders=window.renderOrdersV856=window.renderOrdersV853=window.renderOrdersV857;
  window.setOrderPageSize=(v)=>{state.pagination=state.pagination||{};state.pagination.orders=state.pagination.orders||{page:1,size:10};state.pagination.orders.size=Number(v)||10;state.pagination.orders.page=1;window.renderOrdersV857();};
  window.changeOrderPage=(d)=>{state.pagination.orders.page+=Number(d||0);window.renderOrdersV857();};

  window.renderDashboardV857 = async function(){
    page?.('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de órdenes.');
    const rows=await scopedRows857(); state.rows={...(state.rows||{}),...rows}; const {orders,tickets,inventory,users}=rows;
    const urgent=orders.filter(o=>N(o.priority)==='urgente'), late=orders.filter(delayed), pending=orders.filter(o=>!finalOrder(o)), done=orders.filter(finalOrder);
    const cards=isTech()?[['orders','Órdenes',orders.length,'Asignadas a mi usuario.',''],['urgent','Urgentes',urgent.length,'Prioridad crítica','warn'],['delayed','Atrasadas',late.length,'Más de 7 días abiertas','danger'],['tickets','Tickets',tickets.length,'Asignados a mi usuario.','']]:[['users','Usuarios',users.length,'Perfiles autorizados.',''],['orders','Órdenes',orders.length,'Órdenes técnicas registradas.',''],['urgent','Urgentes',urgent.length,'Prioridad crítica','warn'],['delayed','Atrasadas',late.length,'Más de 7 días abiertas','danger'],['tickets','Tickets',tickets.length,'Solicitudes recibidas',''],['inventory','Inventario',inventory.length,'Activos e insumos','']];
    const st={}; orders.forEach(o=>{const s=statusName(o); st[s]=(st[s]||0)+1;});
    const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, first=new Date(cal.year,cal.month,1), days=new Date(cal.year,cal.month+1,0).getDate(); let cells=''; for(let i=1;i<(first.getDay()||7);i++)cells+='<div class="cal-cell muted"></div>'; for(let day=1;day<=days;day++){ const os=orders.filter(o=>{const x=new Date(o.received_at||o.created_at); return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day;}); cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span class="cal-os" onclick="event.stopPropagation();viewOrderV857('${E(o.id)}')">OS ${E(o.order_number||o.satmanager_order||'')} · ${E(o.office||'-')}</span>`).join('')}</div>`; }
    $('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card ${c[4]}"><small>${E(c[1])}</small><strong>${E(c[2])}</strong><span>${E(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech()?'Resumen de órdenes asignadas al profesional técnico logueado.':'Resumen de órdenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${late.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(st).length?Object.entries(st).map(([k,v])=>`<div class="status-row"><span>${E(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin estados registrados.</p>'}</div><div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Órdenes de Servicio</h2><p>${new Date(cal.year,cal.month,1).toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver órdenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIÉ</span><span>JUE</span><span>VIE</span><span>SÁB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div></div>`;
  };
  window.renderDashboard=window.renderDashboardV856=window.renderDashboardV853=window.renderDashboardV857;
  window.changeDashboardMonth=function(delta){ const n=new Date(), c=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1); state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()}; window.renderDashboardV857(); };
  window.changeDashboardYear=function(y){ const n=new Date(); state.dashboardCalendar=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}; state.dashboardCalendar.year=Number(y)||n.getFullYear(); window.renderDashboardV857(); };

  let lastRoute='', lastAt=0;
  window.route = async function(p){
    const k=alias(p||'dashboard'), now=Date.now();
    if(k===lastRoute && now-lastAt<180) return; lastRoute=k; lastAt=now;
    if(k==='logout'){ try{ await supa.auth.signOut(); }catch(_){} location.href='index.html'; return; }
    if(!$('#content')) window.renderShellV857();
    state.page=k; document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===k));
    if(!window.canModuleV857(k,'view')){ page?.('Acceso restringido',''); $('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este módulo.</p></div>'; return; }
    if(k==='dashboard') return window.renderDashboardV857();
    if(k==='orders') return window.renderOrdersV857();
    if(k==='tickets' && window.renderTicketsV856) return window.renderTicketsV856();
    if(k==='notifications' && window.renderNotificationsV855) return window.renderNotificationsV855();
    if(k==='notifications' && window.renderNotificationsV852) return window.renderNotificationsV852();
    if(window.__routeBefore856) return window.__routeBefore856(k);
  };
  try{ route=window.route; renderDashboard=window.renderDashboardV857; renderOrders=window.renderOrdersV857; renderShell=window.renderShellV857; }catch(_){ }

  setTimeout(()=>{ if(state?.profile && $('#app')){ window.renderShellV857(); window.route(state.page||'dashboard'); } }, 700);
})();
