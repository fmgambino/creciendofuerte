...
// Si el socio todavía está pendiente de activación, no se exponen ganancias ni capital.
if(($u['role']??'')==='socio'){
  try{
    $stPending=db()->prepare('SELECT status, kyc_status FROM partners WHERE user_id=? OR email=? LIMIT 1');
    $stPending->execute([(int)$u['id'], $u['email'] ?? '']);
    $ps=$stPending->fetch();
    $isPending=(($u['status']??'')!=='active') || (($ps['status']??'')!=='active') || (($ps['kyc_status']??'')!=='aprobado');
    if($isPending){
      out(true,[
        'pending_activation'=>true,
        'pending_activation_text'=>app_setting('pending_activation_text','Tu cuenta fue registrada correctamente y quedó pendiente de activación por el SuperAdmin. Este proceso puede demorar hasta 48 hs hábiles.'),
        'stats'=>[
          'capital'=>0,'broker_total'=>0,'partners_pool'=>0,'per_partner'=>0,'partners_available'=>0,
          'paid_withdrawals'=>0,'gains'=>0,'partners'=>0,'pending_withdrawals'=>0
        ],
        'trend'=>[['label'=>date('d/m'),'value'=>0]],
        'chart_type'=>$type
      ]);
    }
  }catch(Throwable $e){}
...
$stPaid=db()->prepare("SELECT COALESCE(SUM(w.amount_usd),0) c FROM withdrawals w WHERE w.status IN('pagado','acreditado','transferido')"); $stPaid->execute(); $paidWithdrawals=(float)$stPaid->fetch()['c'];
...
if(($u['role']??'')==='socio' && $myPartnerId){
  ...
  // Fórmula corregida: Mis Ganancias Acumuladas = Ganancia global por socio - retiros pagados/acreditados/transferidos.
  // Antes se usaba SUM(distributions), lo que podía quedar desactualizado si las distribuciones históricas no reflejaban el global vigente.
  $st=db()->prepare("SELECT COALESCE(SUM(amount_usd),0) FROM withdrawals WHERE partner_id=? AND status IN('pagado','acreditado','transferido')"); $st->execute([$myPartnerId]); $paidMine=(float)$st->fetchColumn();
  $row['gains']=max(0,round($perPartner-$paidMine,2));
  db()->prepare('UPDATE partners SET gains_usd=? WHERE id=?')->execute([$row['gains'],$myPartnerId]);
