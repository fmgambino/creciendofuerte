...
try{
 if($entity==='users'){
  ...
  if($id){
    $oldUser=null; try{ $stOld=db()->prepare('SELECT full_name,email,role,status FROM users WHERE id=? LIMIT 1'); $stOld->execute([$id]); $oldUser=$stOld->fetch(); }catch(Throwable $e){}
    if($plainPassword !== ''){
    ...
    if($oldUser && ($oldUser['status']??'')!=='active' && $fields['status']==='active'){
      crm_send_template($fields['email'],'Cuenta aprobada - 10K CRM','cuenta_aprobada',['titulo'=>'Cuenta aprobada','nombre'=>$fields['full_name'],'mensaje'=>'Tu cuenta fue aprobada por el SuperAdmin. Ya podés ingresar al CRM y visualizar la información habilitada para tu perfil.']);
    }
  } else {
    ...
    notify('Nuevo usuario','Se creó el usuario '.$fields['full_name'],'usuario',null,null,'admin'); crm_email_superadmins('Nuevo usuario registrado - 10K CRM','nuevo_usuario',['titulo'=>'Nuevo usuario registrado','nombre'=>'Administrador','mensaje'=>'Se creó el usuario: '.$fields['full_name'].' · Email: '.$fields['email']]);
  }
 }
 elseif($entity==='partners'){
  ...
  if($id){
    $oldPartner=null; try{ $stOld=db()->prepare('SELECT full_name,email,status,kyc_status FROM partners WHERE id=? LIMIT 1'); $stOld->execute([$id]); $oldPartner=$stOld->fetch(); }catch(Throwable $e){}
    ...
    if($oldPartner && (($oldPartner['status']??'')!=='active' || ($oldPartner['kyc_status']??'')!=='aprobado') && $vals[9]==='active' && $vals[8]==='aprobado'){ crm_send_template($vals[2],'Cuenta de socio aprobada - 10K CRM','cuenta_aprobada',['titulo'=>'Cuenta de socio aprobada','nombre'=>$vals[1],'mensaje'=>'Tu cuenta de socio fue aprobada por el SuperAdmin. Ya podés ingresar al CRM y visualizar tus ganancias acumuladas.']); }
  } else {
    ...
    $pid=(int)db()->lastInsertId(); ensure_partner_user($pid,$vals[1],$vals[2],$vals[9]); notify('Nuevo socio','Se creó el socio '.$vals[1],'socio',null,null,'admin'); crm_email_superadmins('Nuevo socio registrado - 10K CRM','nuevo_socio',['titulo'=>'Nuevo socio registrado','nombre'=>'Administrador','mensaje'=>'Se creó el socio: '.$vals[1].' · Email: '.$vals[2]]); if($vals[9]==='active') crm_send_template($vals[2],'Cuenta de socio creada - 10K CRM','cuenta_creada',['titulo'=>'Cuenta de socio creada','nombre'=>$vals[1],'mensaje'=>'Tu cuenta fue registrada. Ya podés ingresar al CRM.']);
  }
 }
 elseif($entity==='referrals'){
  ...
  else { db()->prepare('INSERT INTO referrals(referrer_partner_id,referred_name,referred_email,capital_usd,commission_percent,status) VALUES(?,?,?,?,?,?)')->execute($vals); notify('Nuevo referido','Se registró '.$vals[1],'referido',null,(int)$vals[0],'partner'); notify('Nuevo referido','El socio registró un referido.','referido',null,null,'admin'); crm_email_superadmins('Nuevo referido registrado - 10K CRM','nuevo_referido',['titulo'=>'Nuevo referido registrado','nombre'=>'Administrador','mensaje'=>'Se registró el referido: '.$vals[1].' · Email: '.$vals[2]]); crm_email_partner((int)$vals[0],'Nuevo referido registrado - 10K CRM','nuevo_referido',['titulo'=>'Nuevo referido registrado','mensaje'=>'Registraste correctamente el referido: '.$vals[1].'.']); }
 }
 elseif($entity==='broker_profits'){
  ...
  else { db()->prepare('INSERT INTO broker_profits(profit_date,gross_profit_usd,master_share_usd,partners_share_usd,notes) VALUES(?,?,?,?,?)')->execute([$date,$gross,$master,$partners,$notes]); $pid=db()->lastInsertId(); $active=db()->query('SELECT id, full_name, email FROM partners WHERE status="active" ORDER BY id ASC')->fetchAll(); $count=count($active); if($count>0){$pct=round(100/$count,4); $amt=round($partners/$count,2); foreach($active as $p){ db()->prepare('INSERT INTO distributions(broker_profit_id,partner_id,amount_usd,percent_share,status) VALUES(?,?,?,? ,"acreditado")')->execute([$pid,$p['id'],$amt,$pct]); db()->prepare('UPDATE partners SET gains_usd=gains_usd+? WHERE id=?')->execute([$amt,$p['id']]); }} sync_partner_gains(); crm_email_superadmins('Nueva ganancia broker cargada - 10K CRM','nueva_ganancia_broker',['titulo'=>'Nueva ganancia broker cargada','nombre'=>'Administrador','mensaje'=>'Se cargó una ganancia broker por US$ '.number_format($gross,2,',','.').' con reparto a socios por US$ '.number_format($partners,2,',','.')]); foreach($active as $p){ notify('Repartición acreditada','Se acreditó la ganancia broker en tu cuenta.','distribucion',null,(int)$p['id'],'partner'); crm_email_partner((int)$p['id'],'Nueva repartición de ganancias acreditada - 10K CRM','nueva_reparticion_ganancias',['titulo'=>'Nueva repartición de ganancias','mensaje'=>'Se acreditó una nueva repartición por US$ '.number_format(($count>0?round($partners/$count,2):0),2,',','.').' en tu cuenta.']); } notify('Nueva repartición de ganancias','Se distribuyó equitativamente el 40% a los socios activos.','distribucion',null,null,'admin'); }
 }
 ...
 elseif($entity==='withdrawals'){
  ...
  if($isSocio){
    ...
    notify('Nuevo retiro solicitado','Solicitaste retirar US$ '.$amount.'.','retiro',null,$pid,'partner'); crm_email_partner($pid,'Retiro solicitado - 10K CRM','retiro_solicitado',['titulo'=>'Retiro solicitado','mensaje'=>'Tu solicitud de retiro por US$ '.number_format($amount,2,',','.').' fue registrada y queda pendiente de revisión.']); notify('Nuevo retiro solicitado',($u['full_name']??'Un socio').' solicitó retirar US$ '.$amount.'.','retiro',null,null,'admin'); crm_email_superadmins('Nuevo retiro solicitado - 10K CRM','retiro_solicitado',['titulo'=>'Nuevo retiro solicitado','nombre'=>'Administrador','mensaje'=>($u['full_name']??'Un socio').' solicitó retirar US$ '.number_format($amount,2,',','.')]);
  } else {
    ...
    if($id){
      ...
      if($old){
        ...
        if($nowPaid){ sync_partner_gains($pid); notify('Retiro pagado','Se acreditó como pagado/transferido tu retiro de US$ '.$amount.'.','retiro',null,$pid,'partner'); $tpl='retiro_'.$status; if(!file_exists(__DIR__.'/../emails/'.$tpl.'.html')) $tpl='retiro_pagado'; crm_email_partner($pid,'Actualización de retiro - 10K CRM',$tpl,['titulo'=>'Actualización de retiro','mensaje'=>'Tu retiro de US$ '.number_format($amount,2,',','.').' cambió al estado: '.$status.'.']); }
      }
    } else {
      db()->prepare('INSERT INTO withdrawals(partner_id,amount_usd,request_type,destination,status) VALUES(?,?,?,?,?)')->execute([$pid,$amount,$type,$dest,$status]); notify('Nuevo retiro solicitado','Se registró un retiro en tu cuenta.','retiro',null,$pid,'partner'); crm_email_partner($pid,'Retiro registrado - 10K CRM','retiro_solicitado',['titulo'=>'Retiro registrado','mensaje'=>'Se registró un retiro por US$ '.number_format($amount,2,',','.').' con estado: '.$status.'.']); notify('Nuevo retiro solicitado','Se solicitó un retiro.','retiro',null,null,'admin'); crm_email_superadmins('Retiro registrado - 10K CRM','retiro_solicitado',['titulo'=>'Retiro registrado','nombre'=>'Administrador','mensaje'=>'Se registró un retiro por US$ '.number_format($amount,2,',','.').' con estado: '.$status.'.']);
