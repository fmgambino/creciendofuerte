...
const ICON={
 ...
 filter:svg('<path d="M3 5h18"/><path d="M7 12h10"/><path d="M10 19h4"/>'),
 wa:svg('<path d="M20.5 11.8a8.5 8.5 0 0 1-12.7 7.4L3 20.7l1.6-4.6A8.5 8.5 0 1 1 20.5 11.8Z"/><path d="M8.5 8.7c.2 3.3 3 6.1 6.3 6.8l1.5-1.5-2.1-1-1 .8c-1.3-.6-2.3-1.6-2.9-2.9l.8-1-1-2.1-1.6.9Z"/>')
};
...
function renderNotifs(){ let unread=state.notifications.filter(n=>!+n.is_read).length; $('#notifBtn').dataset.count=unread; $('#notifPreview').innerHTML=state.notifications.map(n=>`<div class="notif-item ${+n.is_read?'is-read':''}" data-notif-id="${n.id}" onclick="markNotificationRead(${n.id})"><b>${n.title}</b><small>${n.created_at}</small><p>${n.body}</p></div>`).join('')||'<div class="notif-item">Sin notificaciones</div>'; }
async function markNotificationRead(id){ try{let fd=new FormData();fd.append('id',id);let j=await fetch('api/notification_read.php',{method:'POST',body:fd,credentials:'same-origin'}).then(r=>r.json()); if(j.ok){ state.notifications=state.notifications.map(n=>String(n.id)===String(id)?{...n,is_read:1}:n); renderNotifs(); if(state.current==='notificaciones') tablePage('notificaciones'); }}catch(e){} }
async function route(r, push=true){ const aliases={'usuarios':'users','ganancias-broker':'profits','niveles-capital':'capitales','configuracion':'configuraciones','configuraciones':'configuraciones'}; r=aliases[r]||r||'dashboard'; if(!perm(r,'can_view')) r='dashboard'; state.current=r; if(push){ const local=['localhost','127.0.0.1'].includes(location.hostname); if(local) history.replaceState(null,'','./index.php?view='+r); else history.pushState(null,'','./'+r); } $('#pageTitle').textContent=titles[r]||r; const dpt=$('#desktopPageTitle'); if(dpt)dpt.textContent=titles[r]||r; $$('.navitem').forEach(i=>i.classList.toggle('active',i.dataset.route===r)); $$('.bottom-item').forEach(i=>i.classList.toggle('active',i.dataset.route===r)); renderNotifs(); if(r!=='dashboard' && $('#filterFloat')) $('#filterFloat').classList.add('hidden'); if(r==='dashboard') return dashboard(); if(r==='perfil') return profile(); if(r==='roles') return roles(); if(r==='configuraciones') return configuraciones(); return tablePage(r); }
async function dashboard(){
 ...
 $('#content').innerHTML=`<section id="dashStats" class="grid cards"></section><section class="dashboard-filters panel mobile-collapsed" id="dashboardFilters"><div class="filter-row"><div><label>Desde</label><input type="date" id="fFrom" value="${from}"></div><div><label>Hasta</label><input type="date" id="fTo" value="${today}"></div><div><label>Socio</label><select id="fPartner"><option value="">Todos</option>${partnerOptions}</select></div><div><label>Referente</label><select id="fRef"><option value="">Todos</option>${partnerOptions}</select></div><div><label>Gráfico</label><select id="fChart"><option value="line">Línea</option><option value="bar">Barra</option><option value="pie">Torta</option></select></div><button class="btn yellow" id="applyFilters">Aplicar filtros</button></div></section><section class="panel chart-panel"><div class="toolbar"><div><h2 id="chartTitle">Evolución de ganancias broker</h2><p>Filtrá por período, socio, referente y tipo de gráfico.</p></div></div><canvas id="chart" height="120"></canvas></section>`; if($('#filterFloat'))$('#filterFloat').classList.remove('hidden');
...
async function loadDashboardData(){
 ...
 document.querySelectorAll('.pending-activation').forEach(e=>e.remove());
 if(j.pending_activation || state.user.pending_activation){
   const msg=j.pending_activation_text || state.user.pending_activation_text || 'Tu cuenta quedó pendiente de activación por el SuperAdmin. Este proceso puede demorar hasta 48 hs hábiles.';
   const filters=$('#dashboardFilters'); if(filters) filters.classList.add('hidden');
   const chartPanel=$('.chart-panel'); if(chartPanel) chartPanel.classList.add('hidden');
   $('#dashStats').insertAdjacentHTML('beforebegin', `<section class="panel module-note pending-activation"><b>Cuenta pendiente de activación</b><p>${msg}</p><p>Hasta la aprobación, tus importes se mostrarán en cero y solo tendrás acceso al Dashboard y Mi Perfil.</p></section>`);
 }
 if(role==='socio'){
  $('#dashStats').innerHTML=`<div class="card"><span>Mi aporte societario</span><strong>US$ ${money(s.capital)}</strong></div><div class="card broker-card"><span>Ganancias del Broker</span><strong>US$ ${money(s.broker_total)}</strong></div><div class="card success-card"><span>Mis ganancias acumuladas</span><strong>US$ ${money(s.gains)}</strong></div><div class="card global-card"><span>Ganancia global por socio</span><strong>US$ ${money(s.per_partner)}</strong></div><div class="card"><span>Mis retiros pendientes</span><strong>${s.pending_withdrawals||0}</strong></div>`;
 } else {
...
function renderRows(entity,routeName){
 ...
 $('#dataTable').innerHTML=`<thead><tr><th><input type="checkbox" id="checkAll"></th>${keys.map(k=>`<th>${label(k)}</th>`).join('')}<th>Acciones</th></tr></thead><tbody>${pageRows.map(r=>`<tr class="${entity==='notifications'?'notification-row '+(+r.is_read?'is-read':''):''}" ${entity==='notifications'?`onclick="markNotificationRead(${r.id})"`:''}><td><input class="row-check" type="checkbox" value="${r.id}" onclick="event.stopPropagation()"></td>${keys.map(k=>`<td>${fmt(k,r[k])}</td>`).join('')}<td onclick="event.stopPropagation()">${actionBtns(entity,routeName,r)}</td></tr>`).join('')||`<tr><td colspan="${keys.length+2}">Sin registros</td></tr>`}</tbody>`;
...
async function configuraciones(){
 ...
 $('#content').innerHTML=`<section class="panel settings-panel"><div class="toolbar"><div><h2>Configuraciones de la PWA</h2><p>Personalización visual, identidad, logo, favicon, icono instalable y logo de emails.</p></div><button class="btn yellow" id="saveSettings" type="button">Guardar configuración</button></div>
 <form id="settingsForm" class="settings-grid">
  ...
  <div class="field full"><label>Texto del footer</label><input name="footer_text" value="${c.footer_text||''}"></div>
  <div class="field full"><label>URL del desarrollador / footer</label><input name="footer_url" value="${c.footer_url||''}"></div>
  <div class="field full"><label>Texto para socios pendientes de activación</label><textarea name="pending_activation_text" rows="3">${c.pending_activation_text||'Tu cuenta fue registrada correctamente y quedó pendiente de activación por el SuperAdmin. Este proceso puede demorar hasta 48 hs hábiles.'}</textarea></div>
  <div class="field"><label>Activar SMTP / PHPMailer</label><select name="smtp_enabled"><option value="0" ${String(c.smtp_enabled||'0')==='0'?'selected':''}>No</option><option value="1" ${String(c.smtp_enabled||'0')==='1'?'selected':''}>Sí</option></select></div>
  <div class="field"><label>SMTP Host</label><input name="smtp_host" value="${c.smtp_host||''}"></div>
  <div class="field"><label>SMTP Puerto</label><input name="smtp_port" value="${c.smtp_port||'587'}"></div>
  <div class="field"><label>SMTP Seguridad</label><select name="smtp_secure"><option value="tls" ${String(c.smtp_secure||'tls')==='tls'?'selected':''}>TLS</option><option value="ssl" ${String(c.smtp_secure||'')==='ssl'?'selected':''}>SSL</option><option value="" ${String(c.smtp_secure||'')===''?'selected':''}>Ninguna</option></select></div>
  <div class="field"><label>SMTP Usuario</label><input name="smtp_username" value="${c.smtp_username||''}"></div>
  <div class="field"><label>SMTP Password</label><input name="smtp_password" type="password" value="${c.smtp_password||''}"></div>
  <div class="field"><label>Email remitente</label><input name="smtp_from_email" value="${c.smtp_from_email||''}"></div>
  <div class="field"><label>Nombre remitente</label><input name="smtp_from_name" value="${c.smtp_from_name||'EL MASTER PROFE CRM'}"></div>
  <div class="field"><label>Email test SMTP</label><input id="smtpTestEmail" type="email" value="${(state.user&&state.user.email)||c.smtp_from_email||''}" placeholder="email@dominio.com"></div>
  <div class="field"><label>Prueba de envío</label><button class="btn primary" id="sendTestEmail" type="button">Enviar email test</button><small>Envía un correo de prueba usando la configuración SMTP guardada.</small></div>
  <div class="field"><label>Fondo login: tipo</label><select name="login_bg_type"><option value="css" ${String(c.login_bg_type||'css')==='css'?'selected':''}>Animación CSS</option><option value="image" ${String(c.login_bg_type||'')==='image'?'selected':''}>Imagen / imagen subida</option><option value="video" ${String(c.login_bg_type||'')==='video'?'selected':''}>Video / YouTube</option></select></div>
  <div class="field full"><label>URL Video/Imagen para bloque derecho del login</label><input name="login_bg_url" value="${c.login_bg_url||''}" placeholder="Imagen: https://.../fondo.webp · Video: https://.../fondo.mp4 o YouTube https://www.youtube.com/watch?v=..."></div>
  <div class="field"><label>Adjuntar imagen de fondo login</label><input name="login_bg" type="file" accept="image/*,.svg"><small>${c.login_bg_path||''}</small></div>
  <div class="module-note full"><b>Compatibilidad fondo login</b><p><b>Animación CSS</b>: ignora URL e imagen subida. <b>Imagen</b>: usa primero la imagen adjunta y, si no existe, la URL. <b>Video</b>: acepta archivos MP4/WebM externos o URL de YouTube como https://www.youtube.com/watch?v=QQ1QOlMRXNs.</p></div>
 ...
 <div class="module-note"><b>Plantillas de email</b><p>Las plantillas HTML se encuentran en la carpeta <b>/emails</b>. El logo de email se puede cambiar desde esta pantalla.</p><p><b>Gmail SMTP recomendado:</b> Activar SMTP = Sí · Host = smtp.gmail.com · Puerto = 587 · Seguridad = TLS · Usuario = electronicagambino@gmail.com · Password = contraseña de aplicación de Google · Email remitente = electronicagambino@gmail.com.</p></div></section>`;
 
 $('#sendTestEmail').onclick=async()=>{ 
   const to=($('#smtpTestEmail')?.value||'').trim(); 
   if(!to){Swal.fire('Atención','Ingresá un email test válido','warning'); return;} 
   const fd=new FormData(); fd.append('to',to); 
   const r=await fetch('api/test_email.php',{method:'POST',body:fd,credentials:'same-origin'}).then(r=>r.json()).catch(()=>({ok:false,message:'Error de conexión'})); 
   if(r.ok){Swal.fire('Email enviado',r.message||'Prueba SMTP enviada correctamente','success');} else {Swal.fire('Error SMTP',r.message||'No se pudo enviar el email test','error');} 
 };
 $('#saveSettings').onclick=async()=>{ const fd=new FormData($('#settingsForm')); const r=await fetch('api/settings.php',{method:'POST',body:fd,credentials:'same-origin'}).then(r=>r.json()).catch(()=>({ok:false,message:'Error de conexión'})); if(r.ok){state.settings=r.settings; Swal.fire('Listo','Configuración guardada correctamente','success');} else Swal.fire('Error',r.message||'No se pudo guardar','error'); };
}

function toggleDashboardFilters(){ const f=$('#dashboardFilters'); if(!f)return; f.classList.toggle('mobile-collapsed'); f.scrollIntoView({behavior:'smooth',block:'start'});}
