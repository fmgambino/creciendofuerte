...
'notifications'=>"SELECT * FROM notifications WHERE audience IN('admin','all') ORDER BY id DESC",
...
sync_partner_gains($myPartnerId);
if(($u['role']??'')==='socio' && $myPartnerId){
 ...
 if($entity==='notifications'){ $sql="SELECT * FROM notifications WHERE audience='partner' AND (partner_id=? OR user_id=?) ORDER BY id DESC"; $params=[$myPartnerId,(int)$u['id']]; }
}
