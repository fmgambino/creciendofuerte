...
if(($u['role']??'')==='socio' && $myPartnerId){
  ...
  $st=db()->prepare('SELECT COALESCE(SUM(amount_usd),0) FROM distributions WHERE partner_id=?'); $st->execute([$myPartnerId]); $distTotal=(float)$st->fetchColumn();
  ...
  $row['gains']=max(0,round($distTotal-$paidMine,2));
